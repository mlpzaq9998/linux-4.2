...
#ifdef QNAP_SHARE_JOURNAL
static const struct address_space_operations def_blk_journal_aops;
...
int qnap_blkdev_writepages(struct address_space *mapping,
...
long blkdev_fallocate(struct file *filp, int mode, loff_t offset, loff_t len)
{
	int error;
	struct inode *bd_inode = filp->f_mapping->host;
	struct block_device *bdev = I_BDEV(bd_inode);
	...
	/* block device only support fallocate in default mode*/
	if (!q || !q->thindata || mode != FALLOC_FL_QNAP_MAP)
		return -EOPNOTSUPP;

	if (offset + len > i_size_read(bd_inode)) {
		printk(KERN_ERR "%s:%d, over device boundary(%lld) \n",
		       __func__, __LINE__, i_size_read(bd_inode));
		return -EFBIG;
	}

	error = blkdev_issue_preallocate(bdev, offset >> SECTOR_SHIFT,
...
static int blkdev_journal_write_begin(struct file *file,
...
static int blkdev_journal_write_end(struct file *file,
				   struct address_space *mapping,
				   loff_t pos, unsigned len, unsigned copied,
				   struct page *page, void *fsdata)
{
...
int init_blkdev_journal(char *blkdev_name, char *lun_uuid_str)
{
	...
	journal->j_checkpoint_threshold =
		journal_nr_blocks >> BLKDEV_CHECKPOINT_SPACE_SHIFT;
...
{
	if (enable)
		bdev->bd_inode->i_data.a_ops = &def_blk_journal_aops;
	else
...
void check_disk_size_change(struct gendisk *disk, struct block_device *bdev)
{
	...
	if (disk_size != bdev_size) {
		...
		hal_event.arg.action = CHANGE;
		snprintf(hal_event.arg.param.pd_message.pd_name,
		       sizeof(hal_event.arg.param.pd_message.pd_name),
		       "%s", disk->disk_name);
...
static const struct address_space_operations def_blk_journal_aops = {
	...
	.readpages	= blkdev_readpages,
	.writepage	= blkdev_writepage,
	.write_begin	= blkdev_journal_write_begin,
	.write_end	= blkdev_journal_write_end,
	.writepages	= generic_writepages,
	.releasepage	= blkdev_releasepage,
	.direct_IO	= blkdev_direct_IO,
	.is_dirty_writeback = buffer_check_dirty_writeback,
