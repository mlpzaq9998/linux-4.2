                                        ...
                                        struct nfsd3_vstorageargs *args)
{
		...
		args->count=0;
                ...
                break;
	case NFS3_VSTORAGEOP_INQUIRY:
                if (!(p = decode_fh(p, &args->dst))) return 0;
                DBG_PRINT("nfsd: nfs3svc_decode_vstorageargs [INQUIRY] \n");
                break;
	case NFS3_VSTORAGEOP_TASKABORT:
                if (!(p = decode_fh(p, &args->dst))) return 0;
                DBG_PRINT("nfsd: nfs3svc_decode_vstorageargs [TASKABORT] \n");
                break;
        default:
                return 0;
        }
...
/* Patch by QNAP:Workaround bug 37047, nfs(v3) readdir cause duplicate
	filename */
struct {
/* hash will be the same if case insensitive and !NFSD_MAY_64BIT_COOKIE */
	loff_t hash;
	__be32 *buffer;
	int buflen;
} cache_entry_cb;
...
#ifdef QNAP_SEARCH_FILENAME_CASE_INSENSITIVE
/* Patch by QNAP:Workaround bug 37047, nfs(v3) readdir cause
	duplicate filename */
...
#ifdef QNAP_SEARCH_FILENAME_CASE_INSENSITIVE
/* Patch by QNAP:Workaround bug 37047, nfsd(v3) readdir cause
	duplicate filename */
	if (offset != cache_entry_cb.hash) {
                                        ...
                                        struct nfsd3_vstorageres *resp)
{
		...
		case NFS3_VSTORAGEOP_INQUIRY:
			...
			*p++ = htonl(resp->primitives);
                        ...
                        break;
		case NFS3_VSTORAGEOP_TASKABORT:
			DBG_PRINT("nfsd: nfs3svc_encode_vstorageres [%d]\n",
                                resp->operation);
                        *p++ = htonl(operation);
                        *p++ = htonl(resp->primitives);
                        break;
