                                             ...
                                             struct nfsd3_vstorageres *resp)
{
        ...
        case NFS3_VSTORAGEOP_REGISTER:
                ...
                if (argp->magic != 0x8068269529234103LL) {
	...
	case NFS3_VSTORAGEOP_INQUIRY:
		DBG_PRINT("nfsd: VSTORAGE(3) [INQUIRY]\n");
		nfserr = nfsd_inquiry(&argp->dst, &resp->count, &resp->primitives);
		fh_put(&argp->dst);
		break;
	case NFS3_VSTORAGEOP_TASKABORT:
                DBG_PRINT("nfsd: VSTORAGE(3) [TASKABORT]\n");
                nfserr = nfsd_taskabort(&argp->dst, &resp->primitives);
                fh_put(&argp->dst);
                break;
        default:
                nfserr = nfserr_inval;
