...
retry_journal:
	...
	if (ret) {
		...
		if (pos + len > inode->i_size) {
			/* Removed by QNAP:fix sendfile quota issue */
			  ...
			  struct writeback_control *wbc)
{
	...
	/* Qtier support */
	temper = ext4_check_data_temper(inode);
	ext4_io_submit_init(&io_submit, wbc, temper);
			   ...
			   struct writeback_control *wbc)
{
	...
	mpd.wbc = wbc;
	temper = ext4_check_data_temper(inode);
	ext4_io_submit_init(&mpd.io_submit, wbc, temper);
retry:
