				  ...
				  s64 nclusters, unsigned int flags)
{
	...
	/* Patch by QNAP:Fix iSCSI kernel thread use reserved block */
	if (!strncmp(current->comm, ISCSI_RESERVED_NAME ,
		     strlen(ISCSI_RESERVED_NAME)) ||
	    !strncmp(current->comm, RTRR_QSYNCD_RESERVED_NAME ,
		     strlen(RTRR_QSYNCD_RESERVED_NAME))) {
		if (free_clusters - (nclusters + rsv + dirty_clusters) <
				EXT4_B2C(sbi, ISCSI_RESERVED_SIZE))
			return 0;
		else
			return 1;
	}
