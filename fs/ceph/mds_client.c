			       ...
			       u64 features)
{
	...
	*p += info->xattr_len;
	...
	if (features & CEPH_FEATURE_MDS_QUOTA) {
		...
		/*
		 * The following codes read ahead 6 bytes which is used to
		 * deal with compability issue:
		 * DECODE_START_LEGACY_COMPAT_LEN(1, 1, 1, p);
		 * What we really want is the quota info after 6 bytes.
		 */
		ceph_decode_8_safe(p, end, struct_v, bad);
		ceph_decode_8_safe(p, end, struct_compat, bad);
		ceph_decode_32_safe(p, end, struct_len, bad);

		ceph_decode_copy_safe(p, end, &info->quota,
				      struct_len, bad);
	} else
		memset(&info->quota, 0, sizeof(info->quota));
					     ...
					     u64 tid)
{
	...
	BUG_ON(!mutex_is_locked(&mdsc->mutex));
			     ...
			     struct ceph_mds_request *new)
{
	...
	BUG_ON(!RB_EMPTY_NODE(&new->r_node));
...
/* comment temporarily
static int ceph_mds_all_sessions_down(struct ceph_mds_client *mdsc)
{
        struct ceph_mds_session *session;
        int i, ret = 1;
        ...
        for (i = 0; i < mdsc->max_sessions; i++) {
                ...
                if (!session)
                        continue;
                mutex_unlock(&mdsc->mutex);
                state = ceph_mdsmap_get_state(mdsc->mdsmap, i);
			    ...
			    u64 want_flush_tid)
{
	...
	/* comment temporarily
	if (ceph_mds_all_sessions_down(mdsc)) {
		pr_err("all session down!\n");
		ret = 1;
	}*/
	spin_unlock(&mdsc->cap_dirty_lock);
	return ret;
...
ceph_mdsc_create_request(struct ceph_mds_client *mdsc, int op, int mode)
{
	...
	/* comment temporarily
	req->r_timeout = CEPH_REQUEST_TIMEOUT_DEFAULT;
	*/
 ...
 * quota
 */
static void handle_quota(struct ceph_mds_client *mdsc,
			 struct ceph_msg *msg)
{
	struct super_block *sb = mdsc->fsc->sb;
	void *p = msg->front.iov_base;
	...
	ceph_decode_64_safe(&p, end, vino.ino, bad);
	vino.snap = CEPH_NOSNAP;
	inode = ceph_find_inode(sb, vino);
	...
	if (inode) {
		...
		spin_lock(&ci->i_ceph_lock);
		/* i_rctime */
		ceph_decode_copy_safe(&p, end, &rctime,
				      sizeof(struct ceph_timespec), bad);
		ceph_decode_timespec(&ci->i_rctime, &rctime);
		/* i_rbytes, i_rfiles, i_rsubdirs */
		ceph_decode_64_safe(&p, end, ci->i_rbytes, bad);
		ceph_decode_64_safe(&p, end, ci->i_rfiles, bad);
		ceph_decode_64_safe(&p, end, ci->i_rsubdirs, bad);
		/* ceph_quota_info compat payload */
		ceph_decode_8_safe(&p, end, struct_v, bad);
		ceph_decode_8_safe(&p, end, struct_compat, bad);
		ceph_decode_32_safe(&p, end, struct_len, bad);
		/* ceph_quota_info */
		ceph_decode_copy_safe(&p, end, &ci->i_quota, struct_len, bad);
		spin_unlock(&ci->i_ceph_lock);
	}
...
bad:
	spin_unlock(&ci->i_ceph_lock);
	pr_err("error decoding message quota %d\n", err);
	return;
}
