...
bool is_quota_enable(struct ceph_quota_info *quota)
{
	return quota->max_bytes || quota->max_files;
}

static bool ceph_vxattrcb_quota_exists(struct ceph_inode_info *ci)
{
	return is_quota_enable(&ci->i_quota);
}

static size_t ceph_vxattrcb_quota(struct ceph_inode_info *ci,
				  char *val, size_t size)
{
	return snprintf(val, size, "max_bytes=%lld max_files=%lld",
			(long long int)ci->i_quota.max_bytes,
			(long long int)ci->i_quota.max_files);
}

static size_t ceph_vxattrcb_quota_max_bytes(struct ceph_inode_info *ci,
					    char *val, size_t size)
{
	return snprintf(val, size, "%lld", (long long int)ci->i_quota.max_bytes);
}

static size_t ceph_vxattrcb_quota_max_files(struct ceph_inode_info *ci,
					    char *val, size_t size)
{
	return snprintf(val, size, "%lld", (long long int)ci->i_quota.max_files);
}
