				...
				struct ceph_mon_generic_request *req)
{
	...
	if (!req->timeout) {
		...
		timeleft = wait_for_completion_interruptible_timeout(
						&req->completion, req->timeout);
		if (timeleft > 0)
			err = 0; /* successfully complete */
		else if (!timeleft)
			err = -EIO; /* timeout */
		else
			err = timeleft; /* being abort, -ERESTARTSYS */
	}
...
int ceph_monc_do_statfs(struct ceph_mon_client *monc, struct ceph_statfs *buf)
{
	...
	/*setup timeout for statfs */
	/* comment temporarily
	req->timeout = CEPH_REQUEST_TIMEOUT_DEFAULT;
	*/
