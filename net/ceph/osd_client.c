...
static void abort_request(struct ceph_osd_client *osdc,
			  struct ceph_osd_request *req, int err)
{
	pr_err("%s req %p tid %llu err %d\n", __func__, req, req->r_tid, err);
...
static void handle_timeout(struct work_struct *work)
{
	...
	/* scan the mapped requests in each ongoing osd list */
	for (n = rb_first(&osdc->osds); n; n = rb_next(n)) {
		osd = rb_entry(n, struct ceph_osd, o_node);

		list_for_each_entry_safe(req, nreq, &osd->o_requests, r_osd_item) {
			if (opts->osd_request_timeout &&
			    req->r_start_stamp &&
			    time_before(req->r_start_stamp, expiry_cutoff)) {
				pr_err_ratelimited("tid %llu on osd%d timeout\n",
						   req->r_tid, osd->o_osd);
				abort_request(osdc, req, -ETIMEDOUT);
			}
		}
	}

	/* scan the requests with no target osd */
	list_for_each_entry_safe(req, nreq, &osdc->req_notarget, r_req_lru_item) {
		if (opts->osd_request_timeout &&
		    req->r_start_stamp &&
		    time_before(req->r_start_stamp, expiry_cutoff)) {
			pr_warn_ratelimited("tid %llu with no null osd timeout\n", req->r_tid);
			abort_request(osdc, req, -ETIMEDOUT);
		}
	}

	for (n = rb_first(&osdc->requests); n; n = rb_next(n)) {
		req = rb_entry(n, struct ceph_osd_request, r_node);
		...
		if (opts->osd_request_timeout &&
		    ...
		    req->r_start_stamp &&
		    time_before(req->r_start_stamp, expiry_cutoff)) {
			list_move_tail(&req->r_req_lru_item,
				       &osdc->req_notarget);
		}
	}
