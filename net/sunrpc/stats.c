...
#define NFS_SENDPAGE_NOTLAST "nfs_sendpage_notlast"
unsigned int sendpage_notlast_flags=1;
...
static int payload_show(struct seq_file *seq, void *v)
{
	...
	else if(!strcmp(payload->name, NFS_SENDPAGE_NOTLAST))
	    seq_printf(seq, "%u\n", sendpage_notlast_flags);
	return 0;
}
...
static ssize_t nfs_sendpage_notlast_write(struct file *file, const char __user *input,
        size_t size, loff_t *loff)
{
        char buf[128];
        uint32_t payload_size;
        ...
        if (*loff != 0)
                return -ESPIPE;
        payload_size = my_atou(buf);
	if(payload_size == 1 || payload_size == 0){
		sendpage_notlast_flags=payload_size;
        }else{
                printk("invalid value %u to nfs_sendpage_notlast (should be 0 or 1)\n",
                        payload_size);
        }
...
static const struct file_operations nfs_sendpage_notlast_fops = {
        .owner = THIS_MODULE,
        .open = payload_open,
        .read  = seq_read,
        .write = nfs_sendpage_notlast_write,
        .llseek = seq_lseek,
        .release = single_release,
};
...
int rpc_proc_init(struct net *net)
{
	struct sunrpc_net *sn;
#ifdef CONFIG_MACH_QNAPTS
	struct qnap_nfs_payload *tcp_payload, *udp_payload, *nfs_sendpage_notlast;
#endif
	...
	//NFS SENDPAGE_NOTLAST
	nfs_sendpage_notlast = kmalloc(sizeof(*nfs_sendpage_notlast), GFP_ATOMIC);
        if(nfs_sendpage_notlast == NULL) {
                sn->proc_net_rpc_nfs_sendpage_notlast = NULL;
		kfree(tcp_payload);
		kfree(udp_payload);
                return -ENOMEM;
        }
        snprintf(nfs_sendpage_notlast->name, sizeof(nfs_sendpage_notlast->name), NFS_SENDPAGE_NOTLAST);
        sn->proc_net_rpc_nfs_sendpage_notlast = proc_create_data(nfs_sendpage_notlast->name, paylod_perm, sn->proc_net_rpc, &nfs_sendpage_notlast_fops, nfs_sendpage_notlast);
        if(sn->proc_net_rpc_nfs_sendpage_notlast == NULL) {
                kfree(udp_payload);
		kfree(tcp_payload);
		kfree(nfs_sendpage_notlast);
                return -ENOMEM;
        }
#endif /* CONFIG_MACH_QNAPTS */
	return 0;
...
void rpc_proc_exit(struct net *net)
{
	...
	remove_proc_entry("rpc/nfs_sendpage_notlast", net->proc_net);
#endif
