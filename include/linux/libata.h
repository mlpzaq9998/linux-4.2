...
#ifdef CONFIG_MACH_QNAPTS
#define MAX_BURST_ERR_RECORD_NUM		32			// The array size we use to record the jiffies when ata link burst error happens
#define MAX_TOLERANT_BURST_ERR_COUNT	15			// The burst error threshold we set, which refers to the Serial Transport(ATA8-AST) section 5.5.3. An error-burst-rate threshold is detected when 15 error bursts occur in a 5-minute period.
#define JIFFIES_HALF_HOUR				30*60*1000
#define JIFFIES_5_MINUTES				5*60*1000	// An error-burst-rate threshold is detected when 15 error bursts occur in a 5-minute period.
#define JIFFIES_3_SECONDS				3*1000
#define JIFFIES_6_SECONDS				6*1000
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	atomic_t cur_burst_err_ring_buf_idx;
	unsigned long jiffies_at_burst_err[MAX_BURST_ERR_RECORD_NUM];
	unsigned long jiffies_at_bit_err;
	spinlock_t ata_link_check_burst_lock;
	unsigned int link_spd_retry;
#endif
};
...
#ifdef CONFIG_MACH_QNAPTS
	void (*qnap_err_handling)(struct ata_port *ap, struct ata_link *link);
	void (*qnap_mark_ata_port_error)(struct ata_port *ap, int status_error);
	void (*qnap_check_ata_host_status)(struct ata_port *ap);
#endif
#ifdef CONFIG_ATA_SFF
	/*
