...
typedef int (*dm_locate_thin_fn) (struct dm_target *ti, locate_thin_callout_fn fn,
                                  sector_t start, sector_t len, void *remap_desc, void **thin);
 ...
 * @IN len     : the request length of next-level data device
 */

typedef int (*invalidate_callback_fn) (struct block_device *next_dev, sector_t start, sector_t len);

typedef int (*dm_invalidate_fn) (struct dm_target *ti, sector_t start,
                                 sector_t len, invalidate_callback_fn fn);

typedef int (*dm_fast_block_clone_fn) (struct dm_target *ti, THIN_REMAP_DESC *srd,
