...
// Bug#74604 - [ARM AL] change the mtu on pc side cause kernel crash when use L2TP connection.
// there is a bug in the official driver. use patch to fix the bug.
		while (!sg_is_last(sg)) {
            next_sg = sg_next(sg);
            sg = next_sg;
        }
