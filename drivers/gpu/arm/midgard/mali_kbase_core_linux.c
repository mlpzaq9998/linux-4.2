...
static int power_control_init(struct platform_device *pdev)
{
	...
	if (!kbdev)
		return -ENODEV;
			...
			&& defined(CONFIG_REGULATOR) && defined(CONFIG_REGULATOR_G2227)
	//kbdev->regulator = regulator_get_optional(kbdev->dev, "mali");
	kbdev->regulator = regulator_get(NULL, "dcdc3");
	...
	kbdev->clock = clk_get(kbdev->dev, "clk_gpu");
...
#if defined(CONFIG_REGULATOR) && defined(CONFIG_REGULATOR_G2227)
