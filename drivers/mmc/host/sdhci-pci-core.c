...
#if defined(TSX53B) || defined(TSX53BU) || defined(TSX51B) || defined(TSXA51)

/*
 * Workaround for apollolake card detect issue.
 * How to reproduce:
 * 	1. Power on with SD card inserted.
 * 	2. Remove SD card, then power off.
 * 	3. Insert SD card, then power on.
 * 	4. After that, card detect won't work anymore.
 */
static void sdhci_pci_shutdown(struct pci_dev *pdev)
{
	...
	for (i = 0; i < chip->num_slots; i++) {
		...
		irq = slot->host->mmc->slot.cd_irq;
		if (irq >= 0)
			devm_free_irq(&pdev->dev, irq, slot->host->mmc);
	}
}
#else
#define sdhci_pci_shutdown NULL
#endif
