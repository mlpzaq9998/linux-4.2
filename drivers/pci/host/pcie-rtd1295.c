...
static void __iomem	*PCIE_WDT_COUNT_BASE;
...
#define QNAPPCIEWDT_HEADER    0x514E4150

void pcie_wdt_counts_init(void)
{
	writel(0xffffffff, PCIE_WDT_COUNT_BASE);
	writel(0xffffffff, PCIE_WDT_COUNT_BASE + 0x04);

	return;
}
EXPORT_SYMBOL(pcie_wdt_counts_init);

int pcie_wdt_counts_read(void)
{
	unsigned int wdt_count_head = 0;
	unsigned int wdt_count = 0;

	wdt_count_head = readl(PCIE_WDT_COUNT_BASE);
	if(wdt_count_head == QNAPPCIEWDT_HEADER) {
		wdt_count = readl(PCIE_WDT_COUNT_BASE + 0x04);
		return wdt_count;
	}

	return -1;
}
EXPORT_SYMBOL(pcie_wdt_counts_read);

void pcie_wdt_counts_write(void)
{
	unsigned int wdt_count_head = 0;
	unsigned int wdt_count = 0;
	...
	wdt_count_head = readl(PCIE_WDT_COUNT_BASE);
	if(wdt_count_head != QNAPPCIEWDT_HEADER) {
		writel(QNAPPCIEWDT_HEADER, PCIE_WDT_COUNT_BASE);
		writel(0x01, PCIE_WDT_COUNT_BASE + 0x04);
	} else {
		wdt_count = readl(PCIE_WDT_COUNT_BASE + 0x04);
		wdt_count++;
		writel(wdt_count, PCIE_WDT_COUNT_BASE + 0x04);
	}

	return;
}
EXPORT_SYMBOL(pcie_wdt_counts_write);
...
void pcie_gpio_toggle_reset(unsigned int ms)
{
	//Reset PCIE0 device
    printk("pcie1_gpio_toggle_reset ...\n");
...
EXPORT_SYMBOL(pcie_gpio_toggle_reset);
...
static int rtk_pcie_probe(struct platform_device *pdev)
{
	...
	PCIE_WDT_COUNT_BASE = of_iomap(pdev->dev.of_node, 5);
	if (!PCIE_WDT_COUNT_BASE) {
		dev_err(&pdev->dev, "can't request 'PCIE_WDT_COUNT_BASE' address\n");
		return -EINVAL;
	}
