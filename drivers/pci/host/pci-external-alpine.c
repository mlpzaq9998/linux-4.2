...
// patch flow from linux-3.10.20
static int debug_pci = 0;
static struct pci_bus *al_pcie_scan_bus_old(int nr, struct pci_sys_data *sys);

static int s_pcibios_init_resources(int busnr, struct pci_sys_data *sys)
{
    int ret;
...
static u8 s_pci_swizzle_interrupt_pin(const struct pci_dev *dev, u8 pin)
{
    int slot;
...
//dev_info(&dev->dev, "== QC: %s:%s(%d): slot = %d, pin = %d, rc = %d\n", __FILE__, __func__, __LINE__, slot, pin, ( (((pin - 1) + slot) % 4) + 1));
    }

    return (((pin - 1) + slot) % 4) + 1;
}

static u8 s_pci_common_swizzle(struct pci_dev *dev, u8 *pinp)
{
    u8 pin = *pinp;

    while (!pci_is_root_bus(dev->bus)) {
        pin = s_pci_swizzle_interrupt_pin(dev, pin);
        dev = dev->bus->self;
    }
    *pinp = pin;
...
static u8 s_pcibios_swizzle(struct pci_dev *dev, u8 *pin)
{
    ...
    {
        slot = s_pci_common_swizzle(dev, pin);
//dev_info(&dev->dev, "== QC: %s:%s(%d): slot = %d, pin = %d\n", __FILE__, __func__, __LINE__, slot, *pin);
    }
...
static inline struct al_pcie_pd *sys_to_pcie_old(struct pci_sys_data *sys)
{
    return sys->private_data;
}

int s_of_irq_parse_pci(const struct pci_dev *pdev, struct of_phandle_args *out_irq)
{
    ...
    for (;;) {
         ...
         */
        pin = s_pci_swizzle_interrupt_pin(pdev, pin);
        pdev = ppdev;
    }
...
static int al_pcie_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
    ...
    ret = s_of_irq_parse_pci(dev, &oirq);
    if (ret)
        return ret;
...
static void s_pcibios_init_hw(struct device *parent, struct hw_pci *hw,
...
//dev_info(parent, "== QC: %s:%s(%d): bf call hw->setup\n", __FILE__, __func__, __LINE__);
        ret = hw->setup(nr, sys);
        if (ret > 0) {
            ret = s_pcibios_init_resources(nr, sys);
            if (ret)  {
                kfree(sys);
                break;
            }
...
static int al_pcie_setup_old(int nr, struct pci_sys_data *sys)
{
    struct al_pcie_pd *pcie = sys_to_pcie_old(sys);
...
static void s_pdev_fixup_irq(struct pci_dev *dev)
{
    u8 pin, slot = 0;
    int irq = 0;
    int ret;
    ...
    if (pin != 0) {
        slot = s_pci_common_swizzle(dev, &pin);
//dev_info(&dev->dev, "== QC100: %s:%s(%d): slot = %d, new pin = %d\n", __FILE__, __func__, __LINE__, slot, pin);
        irq = al_pcie_map_irq(dev, slot, pin);
//dev_info(&dev->dev, "== QC100: %s:%s(%d): irq = %d\n", __FILE__, __func__, __LINE__, irq);
    }
    dev->irq = irq;
...
static int al_pcie_add_host_bridge(struct al_pcie_pd *pcie)
{
    ...
    hw.setup = al_pcie_setup_old;
    hw.scan = al_pcie_scan_bus_old;
    hw.map_irq = al_pcie_map_irq;
    hw.swizzle = NULL;

    LIST_HEAD(head);
    pci_add_flags(PCI_REASSIGN_ALL_RSRC);
    s_pcibios_init_hw(pcie->dev, &hw, &head);

    for_each_pci_dev(dev)
    {
        if (!strncmp(pci_name(dev), "0001:", 5) || 
            !strncmp(pci_name(dev), "0002:", 5) ||
            !strncmp(pci_name(dev), "0003:", 5))
            s_pdev_fixup_irq(dev);
    }
...
static struct pci_bus *al_pcie_scan_bus_old(int nr, struct pci_sys_data *sys)
{
    ...
    b = pci_create_root_bus(pcie->dev, nr, &al_pcie_ops, pcie, &sys->resources);
    if (!b)
        return NULL;
...
static int al_pcie_probe(struct platform_device *pdev)
{
    ...
    if (err < 0) {
        dev_err(pcie->dev, "failed to enable PCIe controller: %d\n",
            err);
        return err;
    }
#else	
