...
#ifdef CONFIG_MACH_QNAPTS
/* clear FAN_PROG_SEL before set fan control mode */
...
static int __maybe_unused check_CMOS_initialized_by_BIOS(void)
{
	int state;
...
static int f71882fg_probe(struct platform_device *pdev)
{
	...
	/* clear FAN_PROG_SEL before set fan control mode */
	f71882fg_write8(data, F71882FG_REG_FAN_FAULT_TIME,
			f71882fg_read8(data, F71882FG_REG_FAN_FAULT_TIME) & ~0x80);
	wmb();
...
#if defined(TS470U) /* Support Digital IO Board */
		/* set GP50-53 input (GP50,51 Board ID), (GP52,53 PSU-OK) */
		superio_outb(sioaddr, 0xa0, superio_inb(sioaddr, 0xa0) & ~0x0f);
		/* set GP30-37 input (SIO DIN0-DIN7) */
		superio_outb(sioaddr, 0xc0, superio_inb(sioaddr, 0xc0) & 0x00);
		/* set GP42-43 output (SIO DOUT0-DOUT1) */
		superio_outb(sioaddr, 0xb0, superio_inb(sioaddr, 0xb0) | 0x0c);
		/* set GP42-43 push-pull (SIO DOUT0-DOUT1) */
		superio_outb(sioaddr, 0xb3, superio_inb(sioaddr, 0xb3) | 0x0c);
		/* set GP42-43 output low */
		superio_outb(sioaddr, 0xb1, superio_inb(sioaddr, 0xb1) & 0xf3);
#endif
