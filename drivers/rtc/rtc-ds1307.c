...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_ALPINE)
static u8 do_trickle_setup_rx8130(struct i2c_client *client, uint32_t ohms, bool diode);
#endif
...
static int ds1307_get_time(struct device *dev, struct rtc_time *t)
{
	...
	if (ds1307->type == rx_8130) {
		...
		tmp = regmap_read(ds1307->regmap, RX8130_REG_FLAG, &regflag);
		if (tmp) {
	...
	switch (ds1307->type) {
		case rx_8130:
		{
			if (ds1307->regs[DS1307_REG_WDAY] > 0)
			{
				int idx;
				dev_dbg(dev, "== QC: read: org regs[DS1307_REG_WDAY] = 0x%x\n", ds1307->regs[DS1307_REG_WDAY]);
				for (idx =6; idx >= 0; idx--)
				{
					if (ds1307->regs[DS1307_REG_WDAY] & (0x1 << idx))
					{
						ds1307->regs[DS1307_REG_WDAY] = idx + 1;
						break;
					}
				}
				dev_dbg(dev, "== QC: read: chage regs[DS1307_REG_WDAY] = 0x%x, idx = %d\n", ds1307->regs[DS1307_REG_WDAY], idx);
			}
		}
			break;
		default:
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_ALPINE)
    case rx_8130:
		dev_dbg(dev, "== QC: bf set wday = 0x%x from tm_wday = %d\n", buf[DS1307_REG_WDAY], t->tm_wday);
		buf[DS1307_REG_WDAY] = 0x1 << (t->tm_wday%7);
		dev_dbg(dev, "== QC: af set wday = 0x%x from tm_wday = %d\n", buf[DS1307_REG_WDAY], t->tm_wday);
        break;
#endif
	default:
		break;
...
static int rx8130_set_alarm(struct device *dev, struct rtc_wkalrm *t)
{
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_ALPINE)
	dev_dbg(dev, "== QC: rx8130_set_alarm Start\n");
    ret = regmap_bulk_read(ds1307->regmap, 0x10, q, 10);
    if (ret < 0)
        dev_dbg(dev, "== QC: Error (%d): can't read from 0x10 len = 10\n", __LINE__);

    ret = regmap_bulk_read(ds1307->regmap, 0x1C, &q[10], 3);
    if (ret < 0)
        dev_dbg(dev, "== QC: Error (%d): can't read from 0x1C len = 3\n", __LINE__);
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_ALPINE)
	if (ctl[0] & RX8130_REG_EXTENSION_WADA)
	{
		ald[2] = bin2bcd(t->time.tm_mday);
		dev_dbg(dev, "= QC: Month Day : mday = %d, wada = 0x%x\n", t->time.tm_mday, ald[2]);
	}
	else
	{
		ald[2] = 0x1 << (t->time.tm_wday%7);
		dev_dbg(dev, "= QC: Week Day : wday = %d, wada = 0x%x\n", t->time.tm_wday, ald[2]);
	}
	dev_dbg(dev, "%s: write min_alarm = 0x%x, hour_alarm = 0x%x, wada = 0x%x\n", __func__, ald[0], ald[1], ald[2]);
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARCH_ALPINE)
	// debug code
	ret = regmap_write(ds1307->regmap, RX8130_REG_CONTROL0, ctl[2]);
	if (ret < 0)
		return ret;
	dev_dbg(dev, "=== alarm data from user\n");
	dev_dbg(dev, "sec=%d min=%d hour=%d wday=%d mday=%d mon=%d enabled=%d pending=%d\n",
        t->time.tm_sec, t->time.tm_min, t->time.tm_hour,
        t->time.tm_wday, t->time.tm_mday, t->time.tm_mon,
        t->enabled, t->pending);
	dev_dbg(dev, "year=%d yday=%d isdst=%d\n",
		t->time.tm_year, t->time.tm_yday, t->time.tm_isdst);

	dev_dbg(dev, "=== bf set alarm\n");
	dev_dbg(dev, "week:h:m:s     = %02x:%02x:%02x:%02x\n", q[3], q[2], q[1], q[0]);
	dev_dbg(dev, "Y:M:D          = %02x:%02x:%02x\n", q[6], q[5], q[4]);
	dev_dbg(dev, "alarm week:h:m = %02x:%02x:%02x\n", q[9], q[8], q[7]);
	dev_dbg(dev, "ext. = 0x%02x, flg = 0x%02x, ctrl0 = 0x%02x\n", q[10], q[11], q[12]);
    memset(q, 0, sizeof(q));
	...
	dev_dbg(dev, "=== af set alarm\n");
	dev_dbg(dev, "week:h:m:s     = %02x:%02x:%02x:%02x\n", q[3], q[2], q[1], q[0]);
	dev_dbg(dev, "Y:M:D          = %02x:%02x:%02x\n", q[6], q[5], q[4]);
	dev_dbg(dev, "alarm week:h:m = %02x:%02x:%02x\n", q[9], q[8], q[7]);
	dev_dbg(dev, "ext. = 0x%02x, flg = 0x%02x, ctrl0 = 0x%02x\n", q[10], q[11], q[12]);
	dev_dbg(dev, "== QC: rx8130_set_alarm End, ret = %d\n", ret);
	return ret;
#else
 ...
 */
	...
	case rx_8130:
		...
		{
			u8 my_data;
			dev_info(&client->dev, "rx8130_rtc regs init\n");
			my_data = 0x0;
			err = regmap_bulk_write(ds1307->regmap, 0x1c, &my_data, 1);
			if (err < 0)
				return err;

			my_data = 0x20;
			err = regmap_bulk_write(ds1307->regmap, 0x1d, &my_data, 1);
			if (err < 0)
				return err;

			my_data = 0x10;
			err = regmap_bulk_write(ds1307->regmap, 0x1f, &my_data, 1);
			if (err < 0)
				return err;
		}
		want_irq = ds1307->client->irq > 0 && chip->alarm;
		dev_info(&client->dev, "rx8130_rtc check client->irq = %d, chip->alarm = %d, want_irq = %d\n", ds1307->client->irq, chip->alarm, want_irq);
		ds1307_can_wakeup_device = true;
