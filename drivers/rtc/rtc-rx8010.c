...
static int rx8010_read_alarm(struct device *dev, struct rtc_wkalrm *t)
{
...
#if 1 // QNAP Remove
	if (client->irq <= 0){
		printk("=== check: rx8010_read_alarm failed, no irq\n"); // QNAP Debug
		return -EINVAL;
	}
#endif // QNAP Remove
	...
	printk("=== check: %s: minutes:0x%02x hours:0x%02x week/day:0x%02x\n",
		__func__, alarmvals[0], alarmvals[1], alarmvals[2]); // QNAP Debug
	...
	printk("=== check: %s: date: %ds %dm %dh %dmd %dm %dy\n",
		__func__,
		t->time.tm_sec, t->time.tm_min, t->time.tm_hour,
		t->time.tm_mday, t->time.tm_mon, t->time.tm_year); // QNAP Debug
...
static int qnap_rx8010_read_alarm(struct device *dev, struct rtc_wkalrm *t)
{
	struct rx8010_data *rx8010 = dev_get_drvdata(dev);
	struct i2c_client *client = rx8010->client;
	u8 alarmvals[3];		//minute, hour, week/day values
...
#if 0 // QNAP Remove
	printk("== check: rx8010_set_alarm, irq is %d\n",client->irq); // QNAP debug
	if (client->irq <= 0){
		printk("Error: rx8010_set_alarm, irq not defined\n"); // QNAP debug
		return -EINVAL;
	}
#endif // QNAP Remove
...
//	printk("=== checK: %s: write 0x%02x 0x%02x 0x%02x\n", __func__, alarmvals[0], alarmvals[1], alarmvals[2]); // QNAP Debug
...
//	printk("=== checK: %s: set alarm done\n",__func__); // QNAP Debug
	return 0;
}

static int qnap_rx8010_set_alarm(struct device *dev, struct rtc_wkalrm *t)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rx8010_data *rx8010 = dev_get_drvdata(dev);
	u8 alarmvals[3];		//minute, hour, day
...
static int rx8010_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	...
	printk("=== check: call rx8010_ioctl\n"); // QNAP Debug
	//get current extension, flag, control register values
	err = rx8010_read_regs(rx8010->client, RX8010_REG_EXT, 3, ctrl);
	if (err)
		return err;
//	printk("=== check: rx8010_ioctl, cmd is %d, RTC_ALM_SET is %d\n",cmd, RTC_ALM_SET); // QNAP Debug
	...
	case 1:
		//i.e. clear flag
		break;
	case 7: // QNAP Set Alarm (RTC_ALM_SET)
	{
		struct rtc_wkalrm wkalrm,wkalrm_rd;
		struct rtc_time *p_rtc_time =(struct rtc_time *)arg;
		//rx8010_set_alarm(struct device *dev, struct rtc_wkalrm *t)		
		if( copy_from_user(&wkalrm.time,(struct rtc_time *)arg,sizeof(wkalrm.time)) ){
			printk("Error: rx8010_ioctl, set alarm failed\n");
			return -ENOIOCTLCMD;
		}
		printk("=== check: usr = 0x%x sec, copy = 0x%x\n",p_rtc_time->tm_sec,wkalrm.time.tm_sec); // QNAP Debug
//		if (copy_from_user(&qpi, (struct qnap_pic_ioctl *)arg, sizeof(struct qnap_pic_ioctl)))
		wkalrm.enabled = 1;
		wkalrm.pending = 0;
		qnap_rx8010_set_alarm(dev,&wkalrm);
		qnap_rx8010_read_alarm(dev,&wkalrm_rd);
		break;
	}
...
static int rx8010_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("=== check: rtx-rx8010 request_irq: client->irq = %d\n",client->irq); // QNAP Debug
	if (client->irq > 0) {
