...
int gpiod_set_blink(struct gpio_desc *desc, int value)
{
	struct gpio_chip	*chip;
	int			status = -EINVAL;
	...
	if (!chip->set_blink) {
		gpiod_warn(desc,
		       "%s: missing set_blink() operations\n",
		       __func__);
		return -EIO;
	}

	status = chip->set_blink(chip, gpio_chip_hwgpio(desc), value);
	if (status == 0)
		set_bit(FLAG_IS_OUT, &desc->flags);
	trace_gpio_value(desc_to_gpio(desc), 0, value);
	trace_gpio_direction(desc_to_gpio(desc), 0, status);
...
EXPORT_SYMBOL_GPL(gpiod_set_blink);
