...
#ifdef CONFIG_MACH_QNAPTS
		struct t10_pr_registration *tmp_pr_reg;
		tmp_pr_reg = pr_reg;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		/* 1. Increment PRgeneration counter upon a successful
		 *    UNREGISTER, please see spc4r36e, p371,p384
		 *
		 * 2. If the PERSISTENT RESERVE OUT command is terminated due
		 *    to an error or reservation conflict, the PRgeneration
		 *    counter shall not be incremented
		 */
		tmp_pr_reg->pr_res_generation = 
...
#if 0
		pr_info("d_data.pr_iport: %s, d_data.pr_tport: %s, "
			"d_data.reg_isid: %s, d_data.pr_reg_tpgt: %d, "
			"d_data.mapped_lun: %d, d_data.target_lun: %d, "
			"d_data.res_holder: %d\n\n", d_data.i_port, 
			d_data.t_port, d_data.i_sid, d_data.tpgt,
			d_data.mapped_lun, d_data.target_lun, d_data.res_holder
			);
#endif
		new_pr_reg = __core_scsi3_alloc_registration(dev, nacl, 
				lun, deve, d_data.mapped_lun,
