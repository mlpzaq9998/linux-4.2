...
#ifdef CONFIG_MACH_QNAPTS
	/* force go this way.
	 * this shall be depend on your native code, take care this
	 */
	qnap_tmf_tmr_abort_task(dev, tmr, se_sess);
	return;
#endif
	...
	struct list_head *preempt_and_abort_list)
{
	 ...
	 * this shall be depend on your native code, take care this
	 */
	qnap_tmf_tmr_drain_state_list(dev, prout_cmd, tmr_nacl, 
		tas, preempt_and_abort_list);
	return;
#endif
	...
	struct se_cmd *prout_cmd)
{
	...
	if (dev->dev_dr.process_thread)
		qnap_core_tmr_drain_cmd_list(dev, prout_cmd, tmr_nacl,
			tas, preempt_and_abort_list);
#endif
	...
	struct se_cmd *prout_cmd)
{
	...
	if (dr->cluster_api)
		return dr->cluster_api->reset_device(dev, 0);
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	/* force to go our way ... */
	return qnap_core_tmr_lun_reset(dev, tmr,
		preempt_and_abort_list, prout_cmd);
#endif
