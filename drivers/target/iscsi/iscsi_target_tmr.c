	...
	unsigned char *buf)
{
	...
	cmd_count = qnap_iscsit_find_cmd_count_from_itt(conn, hdr->rtt);
	if (cmd_count > 1)
		ref_cmd = qnap_iscsit_find_cmd_from_itt(conn, hdr);
	else
#endif
	...
	if (ref_cmd->immediate_cmd == 1) {
		if (be32_to_cpu(hdr->cmdsn) != be32_to_cpu(hdr->refcmdsn)) {
			pr_err("TMF's RefCmdSN 0x%08x not equal to "
				"TMF's CmdSN 0x%08x. TMF itt:0x%08x "
				"Rejecting ABORT_TASK.\n",
				be32_to_cpu(hdr->refcmdsn), 
				be32_to_cpu(hdr->cmdsn), be32_to_cpu(hdr->itt));
	...
	if (ref_cmd->cmd_sn != be32_to_cpu(hdr->refcmdsn)) {
		...
		struct se_cmd *se_cmd = &ref_cmd->se_cmd;

		pr_err("TMF's RefCmdSN 0x%08x not equal aborted task's "
			"CmdSN 0x%08x. TMF's rtt:0x%08x, "
			"aborted task's itt:0x%08x (iscsi op:0x%x, "
			"scsi op:0x%x). Rejecting ABORT_TASK.\n",
			be32_to_cpu(hdr->refcmdsn), ref_cmd->cmd_sn,
			be32_to_cpu(hdr->rtt), 
			be32_to_cpu(ref_cmd->init_task_tag),
			ref_cmd->iscsi_opcode, 
			((se_cmd->se_cmd_flags & SCF_SE_LUN_CMD) ? \
			se_cmd->t_task_cdb[0] : 0xffffffff));
