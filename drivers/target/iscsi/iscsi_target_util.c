...
#ifdef CONFIG_MACH_QNAPTS
unsigned int iscsi_perf_en=0;
module_param_named(perf_en, iscsi_perf_en, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(perf_en, "a bit mask of performance");
EXPORT_SYMBOL(iscsi_perf_en);
extern unsigned int iscsit_cpumask;
...
#ifdef CONFIG_MACH_QNAPTS
	int old_state = state, where_cmd;
	
	/* this is qnap way and change to TASK_RUNNING if old state is
	 * TASK_INTERRUPTIBLE due to we don't want to sleep in percpu_ida_alloc()
	 * if fail to allocate tag
	 */
...
#ifdef CONFIG_MACH_QNAPTS
	if (tag < 0 ) {
		/* let's us try again to alloc from slab */
		cmd = (struct iscsi_cmd *)qnap_transport_alloc_sess_lio_cmd(
				&se_sess->sess_dr,
                ((old_state == TASK_INTERRUPTIBLE) ? GFP_KERNEL : GFP_ATOMIC));

		if (cmd) {
			tag = 0;
			where_cmd = CMD_FROM_CACHE;
		} else
			tag = -ENOSPC;

	} else
		where_cmd = CMD_FROM_NATIVE_POOL;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	cmd->se_cmd.where_cmd = where_cmd;
	cmd->v_buf_ptr = 0;
	if (iscsit_cpumask != 0xff)
		cmd->se_cmd.dev_cpumask = (1 << smp_processor_id());
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		/* Not need to increase the exp_cmd_sn if immediate command */
		if (sess->icmd_skip_inc_expcmdsn == 1) {
			pr_debug("skip to increase ExpCmdSN for "
				"immediate SCSI cmd\n");
			sess->icmd_skip_inc_expcmdsn = 0;
			...
			unsigned char *buf, __be32 cmdsn)
{
	...
	if (signal_pending(current)) {
		...
		pr_debug("%s: (a) got signal pending\n", __func__);
	...
	case CMDSN_HIGHER_THAN_EXP:
		...
		if (ret < 0) {
#ifdef CONFIG_MACH_QNAPTS
			if (cmd->cmd_flags & ICF_OOO_CMDSN_IGNORED) {
				/* same is duplicated cmdsn for non-OOO cmdsn case */
...
#ifdef CONFIG_MACH_QNAPTS
	if (signal_pending(current)) {
		pr_debug("%s: (b) got signal pending\n", __func__);
		return CMDSN_ERROR_CANNOT_RECOVER;
	}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (se_cmd->where_cmd == CMD_FROM_CACHE)
		qnap_transport_free_sess_lio_cmd(
			&sess->se_sess->sess_dr, (void *)cmd);
	else
#endif
...
static void iscsit_handle_nopin_response_timeout(unsigned long data)
{
	...
	pr_warn("Did not receive response to NOPIN on CID: %hu on"
...
send_hdr:
	...
	while (data_len) {
		...
		if (iscsi_perf_en && ((data_len != sub_len) || cmd->padding || 
