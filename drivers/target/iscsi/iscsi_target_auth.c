	...
	unsigned int *nr_out_len)
{
	...
	unsigned char decode_pwd[CHAP_CHALLENGE_LENGTH];
	size_t compare_len;
	struct iscsi_chap *chap = conn->auth_protocol;
...
#ifdef CONFIG_MACH_QNAPTS
	if (type != HEX)
		qnap_iscsit_chap_base64_to_hex(client_digest, chap_r, strlen(chap_r));
	else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	qnap_iscsit_chap_base64_to_hex(decode_pwd, auth->password, strlen(auth->password));
	sg_init_one(&sg, decode_pwd, strlen(decode_pwd));
	ret = crypto_hash_update(&desc, &sg, strlen(decode_pwd));
#else
...
#ifdef CONFIG_MACH_QNAPTS
		pr_warn("initiator doesn't require target "
			"authentication (rfc3720, p186)\n");
		auth_ret = 0;
#endif
		goto out;
	}
	...
	if (type == BASE64) {
		...
		if (strlen(challenge) % 4) {
			pr_err("challenge len for base64 of CHAP_C is invalid\n");
			goto out;
		}

		/* the encode len is 4*(N chars/3), decode len is 3*(M chars/4) */
		b64_decode_len = strlen(challenge) / 4 * 3;

		/* challenge str not have '0b' or '0B' prefix already and skip 
		 * the padding byte here
		 */
		if (challenge[strlen(challenge) - 1] == '=')
			b64_decode_len--;
		if (challenge[strlen(challenge) - 2] == '=')
			b64_decode_len--;
		...
		qnap_iscsit_chap_base64_to_hex(challenge_binhex, challenge, 
				strlen(challenge));

		if (challenge_binhex[0] == '\0') {
			pr_err("fail to decode base64 of CHAP_C to hex\n");
			goto out;
		}

		challenge_len = b64_decode_len;
	}
	else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	qnap_iscsit_chap_base64_to_hex(decode_pwd, auth->password_mutual, strlen(auth->password_mutual));
	sg_init_one(&sg, decode_pwd, strlen(decode_pwd));
	ret = crypto_hash_update(&desc, &sg, strlen(decode_pwd));
#else
