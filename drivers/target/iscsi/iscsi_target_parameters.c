...
static void iscsi_check_proposer_for_optional_reply(struct iscsi_param *param)
{
	if (IS_TYPE_BOOL_AND(param)) {
		...
		/* mark these due to some hba card (i.e.) doesn't honor these
		 * two key.
		 * - https://www.spinics.net/lists/target-devel/msg14938.html
		 * 
		 * For original purpose to patch globalsan issue, following link
		 * mentions the latest globalsan version will proposal them
		 * - https://www.spinics.net/lists/target-devel/msg14942.html
	...
	struct iscsi_param_list *param_list)
{
	...
	list_for_each_entry(param, &param_list->param_list, p_list) {
		...
		if (!strcmp(param->name, AUTHMETHOD)) {
			 ...
			 */
#ifdef CONFIG_MACH_QNAPTS
			if (ops->MaxRecvDataSegmentLength == 0) {
