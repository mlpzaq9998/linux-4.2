	...
	const char *name)
{
	...
	if (str) {
		...
		*port_str = '\0'; /* Terminate string for IP */
...
#ifdef CONFIG_MACH_QNAPTS
		/* Remember we manually set %ethn as :n right after portal 
		 * in iscsi_create_network_portal() in iscsi_util.
		 */

		/* http://tldp.org/HOWTO/Linux+IPv6-HOWTO/chapter-section-using-api.html:
		 * 
		 * The sin6_scope_id field has an odd use, and it seems that
		 * the IPv6 designers took a huge step backwards when devising
		 * this. 
		 * Apparently, 128-bit IPv6 network addresses are not unique. 
		 * For example, it is possible to have two hosts, on separate
		 * networks, with the same link-local address. 
		 * In order to pass information to a specific host, more than
		 * just the network address is required; the scope identifier
		 * must also be specified. 
		 *
		 * In Linux, the network interface name is used for the scope
		 identifier (e.g. ¡§eth0¡¨) 
		 ...
		 * identifier to produce a "scoped address¡¨. The percent sign
		 * ('%') is used to delimit the network address from the scope
		 * identifier. 
		 * For example, fe80::1%eth0 is a scoped IPv6 address where
		 * fe80::1 represents the 128-bit network address and eth0 is
		 * the network interface (i.e. the scope identifier). Thus, if 
		 * a host resides on two networks, such as Host B in example
		 * below, the user now has to know which path to take in order
		 * to get to a particular host. Host B addresses Host A using
		 * the scoped address fe80::1%eth0, while Host C is addressed
		 * with fe80::1%eth1.
		 *
		 * Host A (fe80::1) - eth0 - Host  - eth1 - Host C (fe80::1)
		 */
		...
		if((ifdev_str = strstr(port_str, ":"))) {
			*ifdev_str = '\0'; /* Terminate string for PORT */
			...
			/* Remove the ifdev index */
			snprintf((char *)name, MAX_PORTAL_LEN, "%s]:%s", buf, 
				port_str);
		}        
#endif
	...
	if (sockaddr.ss_family == AF_INET6)
		snprintf(tpg_np->tpg_np->np_ip, IPV6_ADDRESS_SPACE, "%s", str);
	else
		sprintf(tpg_np->tpg_np->np_ip, "%s", str);
	...
	char *page)
{
	 ...
	 * 2015/11/09 Adam Hsu, force to go this way. Take care this 
	 * from native code
	 */
	return qnap_lio_nacl_show_info(se_nacl, page);
#endif
...
static u32 qnap_iscsi_get_cmdsn(struct se_cmd *se_cmd)
{
	struct iscsi_cmd *cmd = container_of(se_cmd, struct iscsi_cmd, se_cmd);
	return cmd->cmd_sn;
}

static char *qnap_iscsi_get_initiator_name(
	struct se_cmd *se_cmd
	)
{
	struct iscsi_cmd *cmd = container_of(se_cmd, struct iscsi_cmd, se_cmd);
...
#ifdef CONFIG_MACH_QNAPTS
	.get_initiator_name		= qnap_iscsi_get_initiator_name,
	.set_clear_delay_remove		= qnap_lio_tmf_set_clear_delay_remove,
#ifdef ISCSI_D4_INITIATOR
	/* 2009/09/23 Nike Chen add for default initiator */
	.copy_node_attributes		= qnap_lio_copy_node_attributes,
#endif
	.get_task_tag			= iscsi_get_task_tag,
	.get_login_ip			= qnap_iscsi_get_login_ip,
	.get_cmdsn			= qnap_iscsi_get_cmdsn,
	.nl_rt_conf_update		= qnap_nl_rt_conf_update,
	.qnap_iscsi_drop_cmd_from_lun_acl = qnap_iscsi_lio_drop_cmd_from_lun_acl,
#endif
