...
extern int iscsi_perf_en;

#ifdef CONFIG_ISCSI_ERROR_INJECTION
#include "iscsi_error_injection.h"
struct iscsi_error_injection_ops *iscsi_ej_ops = NULL;
EXPORT_SYMBOL(iscsi_ej_ops);
#endif

unsigned int iscsit_cpumask=0xff;
#endif
...
static LIST_HEAD(g_tcp_wait_list);
static DEFINE_SPINLOCK(tcp_wait_lock);
...
static void iscsit_release_wait_list(void)
{
	struct wait_item *item = NULL, *item_tmp = NULL;
	int num = 0;
	spin_lock(&tcp_wait_lock);
	list_for_each_entry_safe(item, item_tmp, &g_tcp_wait_list, list) {
		complete_all(item->comp);
		num++;
	}
	spin_unlock(&tcp_wait_lock);
	pr_debug("[ISCSI] release %d tcp half waiting event(s)\n", num);
}
...
static int __init iscsi_target_init_module(void)
{
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QISCSI_LOG)
	pr_info("iSCSI-Target perf_enhancement %d\n",iscsi_perf_en);
	qnap_nl_create_func_table();
#endif
	...
	qnap_create_acl_notify_info_cache();
	qnap_create_acl_notify_info_wq();
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS 
#ifdef QNAP_KERNEL_STORAGE_V2
	qnap_destroy_acl_notify_info_wq();
	qnap_destroy_acl_notify_info_cache();
#endif
#endif
	/*
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef CONFIG_ISCSI_ERROR_INJECTION
	struct iscsi_sess_ops	*sess_ops = conn->sess->sess_ops;
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		pr_info("to handle SCSI immediate cmd as non-immediate cmd\n");
		conn->sess->icmd_skip_inc_expcmdsn = 1;
#else
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef CONFIG_ISCSI_ERROR_INJECTION
	if (iscsi_ej_ops && (hdr->cdb[0] == READ_6 ||
		hdr->cdb[0] == WRITE_6 ||
		hdr->cdb[0] == READ_10 ||
		hdr->cdb[0] == WRITE_10 ||
		hdr->cdb[0] == READ_12 ||
		...
		hdr->cdb[0] == READ_32 ||
		hdr->cdb[0] == WRITE_32)) {
			iscsi_ej_ops->rx_delay_process(sess_ops->TargetName, sess_ops->InitiatorName, scsilun_to_int(&hdr->lun), conn->cid);
	}
#endif
			  ...
			  bool dump_payload)
{
	...
	check_ret = qnap_iscsit_check_received_cmdsn(conn->sess, 
			be32_to_cpu(hdr->cmdsn));

	/* for case about CMDSN_HIGHER_THAN_EXP, it may happen on
	 * MC/S configuration, so to handle it by normal rule
	 */
	if ((check_ret == CMDSN_LOWER_THAN_EXP)
	|| (check_ret == CMDSN_MAXCMDSN_OVERRUN)
	) 
	{
		   ...
		   struct iscsi_data *hdr)
{
	...
	cmd->se_cmd.digest_zero_copy_skip = true;
	if(qnap_iscsit_zc_splice_work_on_scsi_op(&cmd->se_cmd))
		cmd->se_cmd.digest_zero_copy_skip = false;
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef SUPPORT_ISCSI_ZERO_COPY
	if (qnap_iscsit_check_do_zc_splice(&cmd->se_cmd) == 0){
		rx_got = qnap_iscsit_zc_splice(&cmd->se_cmd, conn->sock, 
			be32_to_cpu(hdr->offset), rx_size);
	} 
	else
#endif
#endif
	...
	/* QNAP: Here not need to take care the case of -ENOSPC for zc splice since 
	 * we did sync i/o solution already */
	if (rx_got != rx_size)
		return -1;
...
#ifdef CONFIG_MACH_QNAPTS
		if (qnap_transport_exec_wt_cmd(&cmd->se_cmd) == 0)
			return 0;
#endif
...
static void iscsit_rx_thread_wait_for_tcp(struct iscsi_conn *conn)
{
	...
	if ((conn->sock->sk->sk_shutdown & SEND_SHUTDOWN) ||
		...
		item = kzalloc(sizeof(struct wait_item), GFP_KERNEL);
		if (!item) {
			pr_err("Unable to allocate a struct tcp_wait_list\n");
		} else {
			pr_debug("[ISCSI] add rx wait to wait list\n");
			spin_lock(&tcp_wait_lock);
			item->comp = &conn->rx_half_close_comp;
			list_add_tail(&item->list, &g_tcp_wait_list);
			spin_unlock(&tcp_wait_lock);
			...
			spin_unlock(&tcp_wait_lock);
			pr_debug("[ISCSI] del rx wait to wait list\n");
...
static int iscsit_send_datain(struct iscsi_cmd *cmd, struct iscsi_conn *conn)
{
	...
	)
	{
		...
		qnap_iscsit_tmf_handle_send_datain(cmd, conn, dr, &eodr, 1);
		/* return 2 */
		return eodr;
	}
	spin_unlock(&se_cmd->tmf_data_lock);
	...
	{
		spin_unlock(&se_cmd->tmf_data_lock);
		qnap_iscsit_tmf_handle_send_datain(cmd, conn, dr, &eodr, 0);
		return eodr;
	}
	spin_unlock(&se_cmd->tmf_data_lock);	
#endif
...
static int iscsit_send_response(struct iscsi_cmd *cmd, struct iscsi_conn *conn)
{
	...
	)
	{
		...
		/* resp_tas - 0, not report TASK ABORTED STATUS
		 * resp_tas - 1, report TASK ABORTED STATUS
		 */
		resp_tas = qnap_iscsit_tmf_handle_send_response(cmd, conn, &ret);
				  ...
				  int skip_bytes, bool *completed)
{
...
#ifdef CONFIG_MACH_QNAPTS
	if (!payload) {
		pr_warn("memory allocation failure from kzalloc() "
			"when build sendtargets buffer, "
			"try vmalloc() again\n");

		/* try use vmalloc() again .. */
		payload = vmalloc(buffer_len);
		if (payload) {
	...
	list_for_each_entry(tiqn, &g_tiqn_list, tiqn_list) {
		...
		/* target ACL supports multiple iSCSI initiators */
		int tmp_acl_count;
		tmp_acl_count = 0;

		tmp_acl_count = qnap_iscsit_get_matched_initiator_count(
					tiqn, conn->sess->sess_ops->InitiatorName);
...
#ifdef CONFIG_MACH_QNAPTS
				int skip, cond1, cond2;
		
				if (inaddr_any)
					continue;
				
				cond1 = (strcmp(conn->login_ip, "::1")
					&& !strcmp(np->np_ip, "::1"));
				
				cond2 = (strcmp(conn->login_ip, "127.0.0.1") 
					&& !strcmp(np->np_ip, "127.0.0.1"));
				
				skip = ((np->np_sockaddr.ss_family == 
						AF_INET6) ? cond1 : cond2);

				if (skip)
				    continue;
				
				cond1 = (NULL != strchr(conn->login_ip,	(int)'.'));
				cond2 = (NULL == strchr(conn->login_ip, (int)'.'));
...
#ifdef CONFIG_MACH_QNAPTS
	int allow_cores = 0;
	int tmp_bitmaps = iscsit_cpumask;

	if (iscsit_cpumask != 0xff) {
		pr_debug("iscsit_cpumask %d\n",iscsit_cpumask);
		while (tmp_bitmaps !=0) {
			allow_cores += tmp_bitmaps%2;
			tmp_bitmaps = tmp_bitmaps/2;
		}

		pr_debug("allow_cores %d\n",allow_cores);
		if (allow_cores > cpumask_weight(cpu_online_mask)) {
			ord = conn->bitmap_id % cpumask_weight(cpu_online_mask);
		} else {
			ord = conn->bitmap_id % allow_cores;
		}
		for_each_online_cpu(cpu) {

			if(!(iscsit_cpumask & (1 << cpu)) ){
				continue; 
			}
...
#ifdef CONFIG_MACH_QNAPTS
	/* case for cmd got TMF and not need to response to host 
	 * send_response won't map iov, so not need do unmap iov again */
	if (ret == 2)
...
err:
#ifdef CONFIG_MACH_QNAPTS
	qnap_iscsit_tmf_clear_dealy_remove(cmd);
#endif
	return -1;
}
	...
	while ((qr = iscsit_get_cmd_from_response_queue(conn))) {
		...
		if (iscsi_ej_ops && cdb) {
			switch (cdb[0]) {
				...
				case WRITE_16:
				case WRITE_32:
					iscsi_ej_ops->tx_delay_process(sess_ops->TargetName, sess_ops->InitiatorName, cmd->se_cmd.orig_fe_lun, conn->cid);
					break;
			}
		}
#endif
...
static int iscsi_target_rx_opcode(struct iscsi_conn *conn, unsigned char *buf)
{
	...
	if (iscsi_ej_ops && (sess_ops != NULL && hdr != NULL)) {
		if(iscsi_ej_ops->random_drop(sess_ops->TargetName, sess_ops->InitiatorName, scsilun_to_int(&hdr->lun), conn->cid))
			return -1;
	}
#endif
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QISCSI_LOG)
	/* 1. Note that sess should not be NULL, otherwise the following operation
	 * will fail.
	 * 2. NOTE: Here now in kernel 3.19 is a little bit different from 3.12.6 
	 */
	qnap_nl_send_post_log(LOGOUT, LOG_INFO, sess, login_ip);
...
#ifdef CONFIG_MACH_QNAPTS
	qnap_iscsit_stop_connection_by_sess(sess);
#else
...
module_param_named(cpumask, iscsit_cpumask, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(cpumask, "iSCSI thread binding CPU");
EXPORT_SYMBOL(iscsit_cpumask);
#endif
