...
/* reduce the size to avoid we may be fail to allocate memory */
#define OFFLOAD_BUF_SIZE	4096
#else
	...
	case ISCSI_OP_SCSI_CMD:
		...
		if (cmd->immediate_data) {
			if (cmd->cmd_flags & ICF_GOT_LAST_DATAOUT) {
				spin_unlock_bh(&cmd->istate_lock);
#ifdef CONFIG_MACH_QNAPTS
				if (!qnap_transport_exec_wt_cmd(&cmd->se_cmd))
					return 0;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	struct iscsi_ooo_cmdsn *__ooo_cmdsn_tmp = NULL;

	/* try check any duplicated cmdsn */
	list_for_each_entry_safe(ooo_cmdsn, __ooo_cmdsn_tmp,
				&sess->sess_ooo_cmdsn_list, ooo_list) {

		if (ooo_cmdsn->cmdsn != cmdsn)
			continue;

		cmd->cmd_flags |= ICF_OOO_CMDSN_IGNORED;
		pr_err("Received cmdsn:0x%08x is duplicated in "
			"ooo cmdsn list, ignoring\n", cmdsn);
		return -EINVAL;
	}
	ooo_cmdsn = NULL;
#endif
...
static void iscsit_handle_dataout_timeout(unsigned long data)
{
	...
	if (!sess->sess_ops->ErrorRecoveryLevel) {
		pr_warn("Unable to recover from DataOut timeout while"
			" in ERL=0.\n");
		goto failure;
	}
