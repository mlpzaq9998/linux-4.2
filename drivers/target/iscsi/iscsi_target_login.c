	...
	u8 zero_tsih)
{
	...
	if (!zero_tsih) {
		...
		qnap_nl_send_post_log(LOGIN_OK, LOG_INFO, sess, login_ip);
#endif
		return;
	}
...
#ifdef CONFIG_MACH_QNAPTS
	/* Allow ipv4 and ipv6 network portal simultaneously.
	 * We have to enable IPV6_V6ONLY option to allow ipv4 and ipv6 network 
	 * portal simultaneously. These two set addresses will bind on the same
	 * port.
...
#ifdef CONFIG_MACH_QNAPTS
	if (!zero_tsih)
		pr_debug("non-zero_tsih case, skip setup sup_prot_ops first\n");
	else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (!zero_tsih) {
		pr_debug("done to setup sup_prot_ops for non-zero tsih case\n");
		conn->sess->se_sess->sup_prot_ops =
			conn->conn_transport->iscsit_get_sup_prot_ops(conn);
	}
#endif
...
new_sess_out:
	...
	if (conn->sess) {
		char login_ip[IPV6_ADDRESS_SPACE];
		strcpy(login_ip, conn->login_ip);
		qnap_nl_send_post_log(LOGIN_FAIL, LOG_ERROR, conn->sess, login_ip); //20515
	}
#endif
	new_sess = true;
old_sess_out:
