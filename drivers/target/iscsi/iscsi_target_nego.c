...
#ifdef CONFIG_MACH_QNAPTS
		/* iSCT Test_9.1
		 * it shall return 'Invalid Request type during Login'
		 */
	...
	struct iscsi_login *login)
{
	...
	list_for_each_entry(param, &conn->param_list->param_list, p_list) {
		if (!strncmp(param->name, SESSIONTYPE, 11)) {
			/* 1. Benjamin 20121120: Patch according to ABC. 
			 *    See RFC 3720 12.21: Default SessionType is Normal.
			 * 2. MAC global san iscsi initiator may not set
			 *    SessionType=Normal for 1st login request ...
			 *
			 */
	...
	struct iscsi_login *login)
{
	...
	while (start < end) {
		 ...
		 * InitiatorName key=value pair.  The initial Login Request of
		 * the first connection of a session MAY also include the
		 * SessionType key=value pair.  For any connection within a 
		 * session whose type is not "Discovery", the first Login 
		 * Request MUST also include the TargetName key=value pair.
		 */
...
#ifdef CONFIG_MACH_QNAPTS
		/* 1st login req may not include session type key but it must
		 * be Normal session if it includes target name key-value pair 
		 */
		if (t_buf)
			goto get_target;
#endif
...
/* Jonathan Ho, 20140416,  one target can be logged in from only one initiator IQN */
	...
	if (!tiqn->cluster_enable) {
		ret = qnap_iscsit_search_tiqn_for_initiator(tiqn, i_buf);
		if (ret == -1) {
			pr_err("Target: %s is connected by other initiator, "
				"login rejected\n", tiqn->tiqn);
...
#ifdef CONFIG_MACH_QNAPTS
	if (ret == 0) {
		qnap_transport_create_sess_lio_cmd_cache(&sess->se_sess->sess_dr,
			sess->session_index, tag_size, __alignof__(struct iscsi_cmd));
	}
#endif
