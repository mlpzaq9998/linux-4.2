...
#ifdef CONFIG_MACH_QNAPTS
static void __qnap_target_do_unmap_work(
	struct work_struct *work
	)
{
	int ret;
	struct se_cmd *cmd = container_of(work, struct se_cmd, unmap_work);

	ret = sbc_execute_unmap(cmd);
...
static sense_reason_t qnap_execute_unmap_wq(
	struct se_cmd *cmd
	)
{
	...
	{
		if (cmd->se_dev->dev_dr.unmap_wq) {
			INIT_WORK(&cmd->unmap_work, __qnap_target_do_unmap_work);
			queue_work(cmd->se_dev->dev_dr.unmap_wq,  &cmd->unmap_work);
			return 0;
		}
	}
...
sbc_emulate_readcapacity_16(struct se_cmd *cmd)
{
	...
	/* if allocation length is zero, it is not error condition */
...
#ifdef CONFIG_MACH_QNAPTS
static sense_reason_t xdreadwrite_callback(struct se_cmd *cmd, bool success, int *post_ret)
#else
...
sbc_parse_cdb(struct se_cmd *cmd, struct sbc_ops *ops)
{
	...
	case SERVICE_ACTION_IN_16:
		...
		case SAI_GET_LBA_STATUS:
			cmd->execute_cmd = qnap_sbc_get_lba_status;
			break;
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
			if (ops->execute_qnap_sync_cache)
				cmd->execute_cmd = ops->execute_qnap_sync_cache;
			else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
		cmd->execute_cmd = qnap_execute_unmap_wq;
#else
...
sbc_execute_unmap(struct se_cmd *cmd)
{
	...
	while (size >= 16) {
		...
		if (test_bit(QNAP_CMD_T_RELEASE_FROM_CONN, &cmd->cmd_dr.cmd_t_state)
			|| qnap_transport_is_dropped_by_tmr(cmd)
		)
		{
			ret = 0;
			goto err;
...
err:
	...
	qnap_transport_check_cmd_hit_thin_threshold(cmd);
