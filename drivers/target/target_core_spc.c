...
#define UNMAP_GRANULARITY_SIZE 0x4000 /* 8MB */
...
// 2009/7/31 Nike Chen change ID to QNAP 
	...
	/* spc4r37a, p354
	 * For VERSION DESCRIPTOR, it is recommened that the first version is
	 * for SCSI architecture standard, followed by the physical transport
	 * standard if any, followed by the SCSI transport protocol, followed
	 * by the appropriate SPC-x version, followed by device type command 
	 * set, followed by a secondary command set if any
	 */
	buf[58] = 0x00;	/* SAM-5, no version claimed */
	buf[59] = 0xa0;

	buf[60] = 0x09; /* iSCSI, no version claimed */
	buf[61] = 0x60;

	buf[62] = 0x04; /* SPC-4, no version claimed */
	buf[63] = 0x60;

	buf[64] = 0x04;	/* SBC-3, no version claimed */
	buf[65] = 0xC0;

	// Benjamin 20110309: Shall return spaces(0x20) instead of zeros 
	// in ASCII field.
	int i;
	...
	buf[4] = 61; /* Set additional length to 61 */
#else
				      ...
				      unsigned char *buf)
{
	...
	if(likely(strcmp(dr->dev_naa, "3.8.1"))) { 
		/* BUG 29894
		 ...
		 * naa body
		 */
		qnap_transport_make_naa_6h_body_old_style(dev, buf);
		return;
	}
#endif
	/*
...
spc_emulate_evpd_83(struct se_cmd *cmd, unsigned char *buf)
{
	...
	/* shall depend on native code to take care this */	
	if(!strcmp(dr->dev_naa, "qnap")) {
		qnap_transport_make_naa_6h_hdr_new_style(&buf[off]);
		off += 3;
...
spc_emulate_evpd_b0(struct se_cmd *cmd, unsigned char *buf)
{
	 ...
	 */
	/* Jira-QTS00000-7308, Windows 2019 request the minimum Optmial Unmap Granularity size 8MB 
	   to do anaylze in Optimizattion Tool (defragment) */
	put_unaligned_be32(UNMAP_GRANULARITY_SIZE, &buf[28]);
...
#ifdef CONFIG_MACH_QNAPTS
	/* this will overwrite following setting */
	if (qnap_sbc_config_tp_on_evpd_b2(dev, buf) == 0)
		return 0;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef SUPPORT_TPC_CMD
	/* page code in supported vpd lists shall be reported in ascending
	 * order beginning with 0x00h
	 */
	{ .page = 0x8f, .emulate = qnap_odx_emulate_evpd_8f },
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS // for read-only snapshot
	if (target_lun_is_rdonly(cmd))
		p[4] |= 0x08;
	else {
		if ((cmd->se_lun) 
		&& (cmd->se_lun->lun_access & TRANSPORT_LUNFLAGS_READ_ONLY)
		)
			p[4] |= 0x08;
	}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	{ .page = 0x08, .subpage = 0x00, .emulate = qnap_spc_modesense_caching },
#ifdef SUPPORT_TP
	{ .page = 0x1c, .subpage = 0x02, .emulate = qnap_sbc_modesense_lbp },
#endif
#else
...
#ifdef CONFIG_MACH_QNAPTS
	atomic_set(&sess->sess_dr.sess_lun_count, lun_count);
	sess->sess_dr.sess_got_report_lun_cmd = true;
#endif
...
spc_parse_cdb(struct se_cmd *cmd, unsigned int *size)
{
...
#ifdef CONFIG_MACH_QNAPTS
		/* overwrite hook qnap modeselect emulation call */
		cmd->execute_cmd = qnap_spc_emulate_modeselect;
#endif
		break;
	case MODE_SELECT_10:
		*size = (cdb[7] << 8) + cdb[8];
...
#ifdef CONFIG_MACH_QNAPTS
		if (cdb[0] == LOG_SENSE)
			cmd->execute_cmd = qnap_spc_logsense;	
#endif
		break;
...
#ifdef SUPPORT_TPC_CMD
		if (IS_TPC_SCSI_PT_OP(cdb[1]))
			cmd->execute_cmd = qnap_odx_pt;
		else if (IS_TPC_SCSI_WUT_OP(cdb[1]))
			cmd->execute_cmd = qnap_odx_wut;
		else
#endif
	...
	case RECEIVE_COPY_RESULTS:
		*size = get_unaligned_be32(&cdb[10]);
#ifdef SUPPORT_TPC_CMD
		if (IS_TPC_SCSI_RRTI_OP(cdb[1]))
			cmd->execute_cmd = qnap_odx_rrti;
		else
#endif
...
#ifdef CONFIG_MACH_QNAPTS
				/* support this command on following lists
				 * (not includes "rd_mcp" backend, refer
				 * core_setup_alua()
				 */
