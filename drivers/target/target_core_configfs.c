...
#ifndef CONFIG_MACH_QNAPTS /* emulate_write_cache becomes read-only */
...
#ifdef CONFIG_MACH_QNAPTS
	da->pi_prot_type = 0;
	pr_warn("not support to store pi_prot_type\n");
	return count;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	pr_warn("not support to store pi_prot_format\n");
	return count;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	if (flag == 0) {
		pr_err("dev[%p]: Not allow to disable APTPL\n", da->da_dev);
		return -EINVAL;
	}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	/* To overwrite the value if qlbs had been set already. 
	 * This is to avoid someone update the value from block_size
	 * hw_block_size directly
	 */
...
#else
TB_DEV_ATTR_RO(target_core, emulate_write_cache);
#endif
...
DEF_TB_QNAP_ATTRIBS(target_core, emulate_v_sup);
DEF_TB_QNAP_ATTRIBS(target_core, emulate_fua_write);
DEF_TB_QNAP_ATTRIBS(target_core, lun_index);
DEF_TB_QNAP_ATTRIBS(target_core, tp_threshold_enable);
DEF_TB_QNAP_ATTRIBS(target_core, tp_threshold_percent);
#ifdef SUPPORT_TP
DEF_TB_QNAP_ATTRIBS_RO(target_core, allocated);
#endif
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	&target_core_dev_attrib_emulate_v_sup.attr,
...
static struct configfs_attribute *target_core_dev_attrs[] = {
	...
	&target_core_attr_dev_naa_vendor.attr,
	&target_core_attr_dev_naa_code.attr,
	&target_core_attr_dev_qlbs.attr,
#ifdef SUPPORT_FAST_BLOCK_CLONE
	&target_core_attr_dev_qfbc.attr,
	&target_core_attr_dev_qfbc_enable.attr,
#endif
	&target_core_attr_dev_wt.attr,
	&target_core_attr_dev_zc.attr,
	&target_core_attr_dev_read_deletable.attr,
#ifdef CONFIG_QTS_CEPH
...
#ifdef CONFIG_MACH_QNAPTS
	if (qlib_create_iorec_cache() != 0)
		goto out;

#ifdef SUPPORT_TPC_CMD
	if (odx_alloc_cache_res() != 0) {
		qlib_destroy_iorec_cache();
		goto out;
	}
#endif
#endif
