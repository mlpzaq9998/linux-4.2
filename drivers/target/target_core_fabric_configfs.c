	...
	struct config_item *item)
{
	...
	atomic_inc_mb(&se_nacl->acl_drop_node);
	spin_unlock(&se_nacl->acl_drop_node_lock);

	spin_lock(&se_nacl->acl_read_info_lock);
	while (atomic_read(&se_nacl->acl_read_info) != 0) {
		spin_unlock(&se_nacl->acl_read_info_lock);
		pr_debug("%s: someone is reading nacl info, to wait\n", 
			__func__);

		init_completion(&comp);
		rc = wait_for_completion_interruptible_timeout(&comp, 
...
_keep_drop_:
	/* after this call, we won't decrease the acl_drop_node again, since
	 * this will remove nacl trees
	 */
#endif
#endif
