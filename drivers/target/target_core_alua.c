...
target_emulate_report_target_port_groups(struct se_cmd *cmd)
{
	...
	/* spc4r37a, p62, p477
	 * If the Allocation Length is zero, it specifies that no data
	 * shall be transferred, this condition shall not e considered an error
	 */
	u32 size;
	size = get_unaligned_be32(&cmd->t_task_cdb[6]);
	if (0 == size) {
		target_complete_cmd(cmd, GOOD);
		return TCM_NO_SENSE;
	}
...
#ifdef CONFIG_MACH_QNAPTS
	/* spc4r37a, p62, p502
	 * If the Parameter List Length is zero, it specifies that no data
	 * shall be transferred, this condition shall not e considered an error
