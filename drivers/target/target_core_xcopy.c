...
static int target_xcopy_gen_naa_ieee(struct se_device *dev, unsigned char *buf)
{
	...
	spc_parse_naa_6h_vendor_specific(dev, &buf[3]);
	return 0;
#endif
				...
				unsigned short tdll)
{
...
#ifdef CONFIG_MACH_QNAPTS
			/* get sbc dev type for src desc */
			if (src)
				sdev_type = (desc[1] & 0x1f);
#endif
			/*
...
#ifdef CONFIG_MACH_QNAPTS
			/* desc[] pointed to next desc already so to get
			 * sbc dev type for dest desc
			 */
			if (!src)
				ddev_type = (desc[1] & 0x1f);
#endif
			ret++;
			break;
		default:
	...
	/* we got the src_dev and dst_dev already, then to check
	 * something finally
	 */
	if (xop->src_dev->transport->get_device_type(xop->src_dev) != sdev_type)
	{
		pr_err("%s: dev type of src dev is not same as Copy Source "
			"Descriptor data\n", __func__);
		return -ENXIO;
	}

	if (xop->dst_dev->transport->get_device_type(xop->dst_dev) != ddev_type)
	{
		pr_err("%s: dev type of dst dev is not same as Copy "
			"Destination Descriptor data\n", __func__);
		return -ENXIO;
	}
#else
	if (rc < 0)
...
sense_reason_t target_do_xcopy(struct se_cmd *se_cmd)
{
...
#ifdef CONFIG_MACH_QNAPTS
	/* Pre-check the tdll and sdll before go to some procedures
	 * - target_xcopy_parse_target_descriptors()
	 * - and, target_xcopy_parse_segment_descriptors()
	 *
	 * In native code, it supports one CSCD descriptor (Copy Source +
	 * Copy Destination) and one Segment descriptor currently (this is for
	 * VMware)
	 */
	int tmp_len0, tmp_len1;

	tmp_len0 = (RCR_OP_MAX_TARGET_DESC_COUNT * XCOPY_TARGET_DESC_LEN);
	tmp_len1 = (RCR_OP_MAX_SG_DESC_COUNT * XCOPY_SEGMENT_DESC_LEN);

	if (tdll != tmp_len0) {
		if (tdll > tmp_len0) {
			pr_err("%s: total CSCD desc counts exceeds max "
				"supported counts: %d\n", __func__,
				RCR_OP_MAX_TARGET_DESC_COUNT);
			ret = TCM_TOO_MANY_TARGET_DESCRIPTORS;
		}
		else {
			pr_err("%s: total CSCD desc len is smaller than "
				"necessary requirement: %d\n", __func__,
				tmp_len0);
	...
	if (sdll != tmp_len1) {
		if (sdll > tmp_len1) {
			pr_err("%s: total SEG desc counts exceeds max "
				"supported counts: %d\n", __func__,
				RCR_OP_MAX_SG_DESC_COUNT);
			ret = TCM_TOO_MANY_TARGET_DESCRIPTORS;
		} else {
			pr_err("%s: total SEG desc len is smaller than "
				"necessary requirement: %d\n", __func__,
				tmp_len1);
			ret = TCM_INVALID_PARAMETER_LIST;
		}
	...
	ret = qnap_fbc_do_fast_block_clone(xop);
	if (ret != -1) {
		pr_debug("XCOPY: do fast block clone\n");
		xcopy_pt_undepend_remotedev(xop);
		...
		pr_debug("<0>XCOPY: do not fast block clone\n");
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	/* Depend on the block size to overwrite the p[37 ~ 39]. 
	 * Original native code is based on block size - 512 bytes
	 */
