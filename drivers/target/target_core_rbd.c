...
static int tcm_rbd_configure_device(struct se_device *dev)
{
	...
	dev->dev_dr.dev_type = QNAP_DT_RBD;
#endif
...
#ifdef CONFIG_MACH_QNAPTS
static void *__qnap_rbd_get_fe_dev(
	struct se_device *se_dev
	)
{
	return (void *)TCM_RBD_DEV(se_dev);
}

static struct request_queue *__qnap_rbd_get_rq(
	struct se_device *se_dev
	)
{
	...
	tcm_rbd_dev = __qnap_rbd_get_fe_dev(se_dev);
	q = bdev_get_queue(tcm_rbd_dev->bd);
	return q;
}
#endif
...
#ifdef CONFIG_MACH_QNAPTS
	.get_dev			= __qnap_rbd_get_fe_dev,
	.get_rq				= __qnap_rbd_get_rq,
#endif
