...
/* {BUG 36179} serial number wrong on 6281/6282/7042 */
static char found_7042;
int Is_Sata_7042(void)
{
	return found_7042;
}
...
static void mv_err_intr(struct ata_port *ap)
{
	...
	struct ata_queued_cmd *qc;
	int abort = 0;
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARM) && defined(CONFIG_ARM64)
...
#endif /* defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_ARM) && defined(CONFIG_ARM64) */
		}
	}
				  ...
				  void __iomem *mmio, unsigned int port)
{
	 ...
	 * WD BookII Compatibility issue, device can't be detected
	 */
...
#if defined(CONFIG_MACH_QNAPTS)
		/** Patch by QNAP: delay SATA disk initialization */
		/* marvell 7042 port 2 port 3 will power on by
		 * order every 5 sec
		 */
		if ((port == 2) || (port == 3)) {
			dev_dbg(&pdev->dev, "Wait %d seconds to initialize scsi %d\n",
