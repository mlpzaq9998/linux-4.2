...
static void ata_port_led(struct ata_port *ap, u8 on)
{
	...
	if (retry == SGPIO_RETRY_MAX)
		ata_port_warn(ap, "%s:SGPIO always busy\n", __func__);
}


#ifdef QNAP_HAL
/* QNAP Enhanced NCQ (ENCQ) enabling scheme */
static ssize_t ata_scsi_ncq_show(struct device *device,
...
int ata_cmd_ioctl(struct scsi_device *scsidev, void __user *arg)
{
...
#if defined(CONFIG_MACH_QNAPTS)
	/* QNAP-Aaron: #30315: if SMART command, set timeout to 30 S */
	if (args[0] == ATA_CMD_SMART)
		cmd_result = scsi_execute(scsidev, scsi_cmd, data_dir, argbuf,
					  argsize, sensebuf, (30*HZ), 5, 0,
					  NULL);
	else
#endif
	...
	/* Fix bug#36179- HDD Smart->Hard Disk Information's Serial
	 * number display wrong on 6281/6282/7042 controller */
#if defined(TS410) || defined(TS419)
extern int Is_Sata_7042(void); /* in sata_mv.c */
	if (Is_Sata_7042())
#endif
	if (argbuf != NULL)
