...
{
	u32 sstatus;
	int i, rc = 0, det_wait = 0;
	ktime_t start, finish;

	ahci_power_up(link->ap);

	/* wait at least 10 usec at first time */
	udelay(10);
	start = ktime_get();
	for (i = 0; i < AL_AHCI_SPEED_AN_TRIES; i++) {
		do {
			...
			if (rc) {
				ata_dev_err(&link->device[0], "Fatal error\n");
				goto out;
			}
			finish = ktime_get();
			...
			if (!(sstatus & 0x1)) {
				if (++det_wait > 10) {
					rc = 0;
					ata_dev_err(&link->device[0],
						    "Error - device not present, sstatus = 0x%x\n",
						    sstatus);
					goto out;
				} else {
					start = ktime_get();
			...
			if (sstatus & 0xF0)
				break;
		} while (ktime_us_delta(finish, start) <= AL_SPEED_NEG_TIME_USEC);

		/* Check negotiated speed */
		if (sstatus & 0xF1) {
			finish = ktime_get();
			if ((sstatus & 0xF0) == 0x30) {
				ata_dev_info(&link->device[0],
					     "success - got 6 Gbps after %d usec\n",
					     (u32)ktime_us_delta(finish, start));
				break;
			} else {
				/* if no speed detected */
				if ((sstatus & 0xF0) != 0) {
					ata_dev_err(&link->device[0],
						    "SSS WA failed after %d usec, sstatus = 0x%x\n",
						    (u32)ktime_us_delta(finish, start),
						    sstatus);
					break;
				}
		...
		if (rc)
			return rc;
		rc = sata_scr_write_flush(link, SCR_ERROR, 0xFFFFFFFF);
		if (rc)
			return rc;
		start = ktime_get();
	}
	if (i > 1)
		ata_dev_warn(&link->device[0],
			     "WARNING: PHY hardreset for port repeated %d times\n",
			     i);
	/* Check if device is present */
	if (sstatus & 0x3) {
		u32 serror;
