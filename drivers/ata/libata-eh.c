...
/* saving time when retrying media errors */
...
#if defined(CONFIG_MACH_QNAPTS) && defined(QNAP_HAL)
	// For TL-SATA CallTrace issue, print warning log if the pointer was NULL
	if (sdev != NULL) {
		if ((sdev->host) != NULL)
			send_event = 1;
		else
			ata_dev_warn(dev, "sdev->host is NULL\n");
	}
	else {
		ata_dev_warn(dev, "sdev is NULL\n");
	}
	...
	for (tag = 0; tag < ATA_MAX_QUEUE; tag++) {
		...
		if (qc->err_mask & AC_ERR_TIMEOUT) {
			for_each_sg(qc->sg, sg, qc->n_elem, si) {
				ata_dev_err(qc->dev,
				 "sg[%d]: dma_address 0x%x, length 0x%x\n",
				 si, sg_dma_address(sg), sg_dma_len(sg));
			}
		}
		 ...
		 ata_reset_fn_t hardreset, ata_postreset_fn_t postreset)
{
 ...
 retry:
	...
	deadline = ata_deadline(jiffies,
					...
					qnap_ata_reset_to_ms :
					ata_eh_reset_timeouts[try++]);
		if (qnap_ata_reset_to_ms)
			try++;
#else
	...
	if (reset) {
		...
		if (reset == hardreset &&
	     ...
	     "follow-up softreset required but no softreset available\n");
			...
			ata_eh_about_to_do(link, NULL, ATA_EH_RESET);
#if defined(CONFIG_MACH_QNAPTS)
			/* BUG 26821, Patch by QNAP:deadline need to reset */
		   ...
		   struct ata_link **r_failed_link)
{
...
#if defined(CONFIG_MACH_QNAPTS)
				/* saving time when retrying media errors */
				ehc->tries[dev->devno] = qnap_ata_retry_val ?
					qnap_ata_retry_val : ATA_EH_DEV_TRIES;
#else
				ehc->tries[dev->devno] = ATA_EH_DEV_TRIES;
#endif
			}
 ...
 retry:
	...
	ata_for_each_link(link, ap, PMP_FIRST) {
		if ((link != NULL) &&
		    (link->device != NULL) &&
		    (link->device->class == ATA_DEV_PMP)) {
			printk("Device class ATA_DEV_PMP, skip ata port error handling!\n");
			skip_qnap_err_handling = 1;
		}
	}
	...
	ata_for_each_link(link, ap, EDGE) {
		...
		if (rc) {
			 ...
			 * if SCR isn't accessible on a fan-out port, PMP needs to be reset
			 */
			if ((rc == -ERESTART) && skip_qnap_err_handling)
			{
				ata_link_err(link, "SCR isn't accessible on a fan-out port, rc %d\n", rc);
			}
			else
			{
				/* HAL notify ATA eh_reset failed  */
				NETLINK_EVT hal_event;
				...
				netlink_pd->ata_print_id = link->ap->print_id;
				send_hal_netlink(&hal_event);
			}
#endif
		...
		if (!skip_qnap_err_handling)
		{
			if (ap->ops->qnap_err_handling)
				ap->ops->qnap_err_handling(ap, link);
		}
#endif
	}
