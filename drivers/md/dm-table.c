...
static int device_qnap_map_capable(struct dm_target *ti, struct dm_dev *dev,
				  sector_t start, sector_t len, void *data)
{
	struct request_queue *q = bdev_get_queue(dev->bdev);

	return q && blk_queue_qnap_map(q);
}

static bool dm_table_supports_qnap_map(struct dm_table *t)
{
	struct dm_target *ti;
	unsigned i = 0;
	...
	while (i < dm_table_get_num_targets(t)) {
		...
		if (!strcasecmp(ti->type->name, "thin") ||
			!strcasecmp(ti->type->name, "thick"))
			return true;

		if (ti->type->iterate_devices &&
		    ti->type->iterate_devices(ti, device_qnap_map_capable, NULL))
			return true;
	}
...
static bool dm_table_enable_encryption(struct dm_table *t)
{
	struct dm_target *ti;
	unsigned i = 0;
...
static bool dm_table_locate_thick(struct dm_table *t)
{
	struct dm_target *ti;
	unsigned i = 0;
			       ...
			       struct queue_limits *limits)
{
	...
	if (!dm_table_supports_qnap_map(t))
		queue_flag_clear_unlocked(QUEUE_FLAG_QNAP_MAP, q);
	else
		queue_flag_set_unlocked(QUEUE_FLAG_QNAP_MAP, q);
