...
static int sm_find_serial_free(void *addr, unsigned begin, unsigned end,
			unsigned *result, int *len)
{
	int found = 0;

	while (begin < end) {
		if (!(begin & (ENTRIES_PER_WORD - 1)) &&
		    bitmap_word_used(addr, begin) && !found) {
			begin += ENTRIES_PER_WORD;
			continue;
		}
		...
		if (found && sm_lookup_bitmap(addr, begin))
			break;
...
int sm_ll_find_serial_free_blks(struct ll_disk *ll, dm_block_t begin,
	dm_block_t end, dm_block_t *result, int *len)
{
	int r;
	struct disk_index_entry ie_disk;
	dm_block_t i, index_begin = begin;
	...
	for (i = index_begin; i < index_end; i++, begin = 0) {
		...
		r = sm_find_serial_free(dm_bitmap_data(blk),
				 max_t(unsigned, begin, le32_to_cpu(ie_disk.none_free_before)),
				 bit_end, &position, &free_blks);
		if (r == -ENOSPC) {
			/*
			 * This might happen because we started searching
			 * part way through the bitmap.
