...
struct dm_space_map {
	...
	/*
	 * Return the number of blocks are available for allocation _now_
	 * for certain range.
	 */
	int (*get_nr_free_range)(struct dm_space_map *sm, 
	...
	/*
	 * Allocate specific data block 
	 */
	int (*alloc_block)(struct dm_space_map *sm, dm_block_t b);
	...
	/*
	 *  You can register boundary. For physical blocks >= b && < e, thery are
	 * not allowed to be allocated.
	 */
	int (*register_boundary)(struct dm_space_map *sm, dm_block_t b, dm_block_t e);

	int (*get_boundary)(struct dm_space_map *sm, dm_block_t *b, dm_block_t *e);
};
...
static inline int dm_sm_get_nr_free_range(struct dm_space_map *sm, 
	dm_block_t begin, dm_block_t end, dm_block_t *count)
{
	if (sm->get_nr_free_range)
		return sm->get_nr_free_range(sm, begin, end, count);
...
static inline int dm_sm_alloc_block(struct dm_space_map *sm, dm_block_t b)
{
	return sm->alloc_block(sm, b);
}
...
static inline int dm_sm_register_boundary(struct dm_space_map *sm, dm_block_t b, dm_block_t e)
{
	if (sm->register_boundary)
		return sm->register_boundary(sm, b, e);

	return -EINVAL;
}

static inline int dm_sm_get_boundary(struct dm_space_map *sm, dm_block_t *b, dm_block_t *e)
{
	if (sm->get_boundary)
		return sm->get_boundary(sm, b, e);
