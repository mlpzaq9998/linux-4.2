...
int mv64xxx_i2c_check_response(struct mv64xxx_i2c_data *drv_data, u8 *ctl, u8 *status)
{
	...
	if (timeout > MAX_RETRIES) {
		*status = 0xff;
		i2c_debug("###MV_9235_I2C: Check response timeout!\n");
		return -ETIMEDOUT;
	}
	return 0;
}
...
//	}
	...
	while (1) {
		...
		if (ret < 0) {
			...
			drv_data->state = MV64XXX_I2C_STATE_INVALID;
			//spin_unlock_irqrestore(&drv_data->lock, flags);
			i2c_debug("###MV_9235_I2C: mv64xxx_i2c_check_response fail, I2C ACTION STATE INVALID...\n");
			return drv_data->rc;
		}
...
int mv9235_i2c_init(struct pci_dev *pdev, void __iomem *pci_bar5)
{
	...
	/* FIXME dev_set_drvdata return void since 2c1f1ff0f */
	/**
	if((rc = dev_set_drvdata(&drv_data->adapter.dev, drv_data)) != 0)
		dev_err(&drv_data->adapter.dev,
				"mv9235: Can't set i2c adapter, rc: %d\n", -rc);
				*/
	dev_set_drvdata(&drv_data->adapter.dev, drv_data);
