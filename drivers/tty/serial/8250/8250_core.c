 ...
 */
#ifdef  CONFIG_QND_INTEL_LPC_WA
extern void intel_lpc_workaround(void);
#endif
...
#if defined(CONFIG_MACH_QNAPTS)
#if defined(CONFIG_ARCH_ALPINE) || defined(CONFIG_ARCH_MVEBU) || defined(CONFIG_ARCH_RTD129X)
void kernel_booting_buzzer(void)
{
#if defined(CONFIG_ARCH_MVEBU) || defined(CONFIG_ARCH_ALPINE)
	struct uart_8250_port *up = &serial8250_ports[1];
	unsigned int i = 0;
	unsigned char buzzer_cmd[4] = { '@', 'C', '2', 0x31};
...
#if defined(CONFIG_ARCH_MVEBU) || defined(CONFIG_ARCH_RTD129X) || defined(CONFIG_ARCH_ALPINE)
	for(i=0;i<sizeof(buzzer_cmd);i++) {
		//printk("buzzer_cmd[%d] = 0x%x\n", i, buzzer_cmd[i]);
		serial_out(up, UART_TX, buzzer_cmd[i]); /* 0.5 sec buzzer cmd */
		udelay(200);
	}
#endif
