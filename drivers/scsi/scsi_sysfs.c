...
#ifdef VIRTUAL_JBOD
extern void qnap_scsi_probe_sd(struct scsi_device *sdev);
#endif
#endif
...
static ssize_t store_qnap_probe_sd(
	struct device *dev,
	...
	)
{
	...
	if (val != 1) {
		pr_warn("invalid val:%d\n", val);
		return count;
	}

	qnap_scsi_probe_sd(sdev);
	return count;
}
static DEVICE_ATTR(qnap_probe_sd, S_IWUSR, NULL, store_qnap_probe_sd);

static ssize_t show_qnap_probe_sd_type(
	struct device *dev,
	struct device_attribute *attr,
	char *buf
	)
...
static ssize_t store_qnap_probe_sd_type(
	struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count
	)
{
	...
	if (qnap_is_iscsi_disk(sdev)) {
		pr_warn("invalid op, not QNAP iSCSI device\n");
		return count;
	}

	if (!(val == SD_VJBOD || val == SD_VJBOD_RDISK)) {
		pr_warn("invalid value:%d\n", val);
		return count;
	}

	if (sdev->probe_sd_done == 1) {
		pr_warn("error, sd was probed done, skip the operaiotn\n");
		return count;
	}

	sdev->probe_sd_type = val;
	return count;
}
static DEVICE_ATTR(qnap_probe_sd_type, S_IRUGO | S_IWUSR, show_qnap_probe_sd_type, store_qnap_probe_sd_type);

static ssize_t show_qnap_probe_sd_done(
	struct device *dev,
	struct device_attribute *attr,
	char *buf
	)
...
static DEVICE_ATTR(qnap_probe_sd_done, S_IRUGO | S_IWUSR, show_qnap_probe_sd_done, NULL);

static ssize_t show_qnap_dev_naa(
	struct device *dev,
	struct device_attribute *attr,
	char *buf
	)
{
	struct scsi_device *sdev = to_scsi_device(dev);
	return snprintf(buf, 0x100, "%s\n",
		((sdev->dev_naa) ? sdev->dev_naa : "nullnullnull"));
}
static DEVICE_ATTR(qnap_dev_naa, S_IRUGO | S_IWUSR, show_qnap_dev_naa, NULL);

static ssize_t show_qnap_dev_sn(
	struct device *dev,
	struct device_attribute *attr,
	char *buf
	)
{
	struct scsi_device *sdev = to_scsi_device(dev);
	return snprintf(buf, 0x100, "%s\n",
		((sdev->dev_sn) ? sdev->dev_sn : "nullnullnull"));
}
static DEVICE_ATTR(qnap_dev_sn, S_IRUGO | S_IWUSR, show_qnap_dev_sn, NULL);

static ssize_t show_qnap_dev_name(
	struct device *dev,
	struct device_attribute *attr,
	char *buf
	)
{
	struct scsi_device *sdev = to_scsi_device(dev);
	return snprintf(buf, 0x100, "%s\n",
		((sdev->dev_name) ? sdev->dev_name: "nullnullnull"));
}
static DEVICE_ATTR(qnap_dev_name, S_IRUGO | S_IWUSR, show_qnap_dev_name, NULL);

static ssize_t show_qnap_dev_size_info(
	struct device *dev,
	struct device_attribute *attr,
	char *buf
	)
{
	...
	return snprintf(buf, 0x100, "%llu-%u\n",
		sdev->dev_capacity, sdev->dev_sector_size);
}
...
{
	struct scsi_device *sdev = container_of(work, typeof(*sdev), remove_work);
...
#ifdef CONFIG_MACH_QNAPTS
		/* 1. Workaroud to avoid that SCSI device removal through sysfs 
		 *    triggers a deadlock. Here ONLY to handle iscsi disk.
		 *    Please refer https://marc.info/?t=147750760900001&r=1&w=2
		 *
		 * 2. Actually, we shall consider it works on all scsi devs but
		 *    nobody complains it except iscsi case ....
		 */
		if (!qnap_is_iscsi_disk(to_scsi_device(dev)))
...
#ifdef CONFIG_MACH_QNAPTS
#ifdef VIRTUAL_JBOD
	&dev_attr_qnap_dev_naa.attr,
	&dev_attr_qnap_dev_sn.attr,
	&dev_attr_qnap_dev_name.attr,
	&dev_attr_qnap_dev_size_info.attr,
	&dev_attr_qnap_probe_sd.attr,
