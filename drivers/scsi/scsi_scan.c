...
#ifdef VIRTUAL_JBOD
extern int qnap_scsi_get_lun_naa(struct scsi_device *sdev);
extern int qnap_scsi_get_lun_capacity(struct scsi_device *sdev);
extern int qnap_scsi_get_lun_sn(struct scsi_device *sdev);
					   ...
					   u64 lun, void *hostdata)
{
	...
	sdev->dev_name = NULL;
	sdev->dev_capacity = 0;
	sdev->dev_sector_size = 0;
	sdev->probe_sd = -1;
	sdev->probe_sd_done = -1;
	sdev->probe_sd_type = -1;
	sdev->add_sg_done = -1;
#endif
#endif
		...
		int *bflags, int async)
{
	...
	if (!qnap_is_iscsi_disk(sdev)) {
		qnap_scsi_get_lun_naa(sdev);
		qnap_scsi_get_lun_capacity(sdev);
		qnap_scsi_get_lun_sn(sdev);
	}
#endif
#endif
