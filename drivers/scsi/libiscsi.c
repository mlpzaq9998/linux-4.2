...
static int qnap_vjbod_iscsi_has_ping_timed_out(struct iscsi_conn *conn)
{
			 ...
			 char *buf)
{
	...
	case ISCSI_HOST_PARAM_INITIATOR_NAME:
		...
		break;
#ifdef VIRTUAL_JBOD
	case ISCSI_HOST_PARAM_QNAP_NETDEV_NAME:
		len = sprintf(buf, "%s\n", ihost->qnap_netdev);
		break;
#endif
	default:
...
#ifdef VIRTUAL_JBOD
	case ISCSI_HOST_PARAM_QNAP_NETDEV_NAME:
		return iscsi_switch_str_param(&ihost->qnap_netdev, buf);
#endif
	default:
		return -ENOSYS;
	}
