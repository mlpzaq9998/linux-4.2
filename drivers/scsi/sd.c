...
#endif /* defined(NAS_VIRTUAL) */

#ifdef VIRTUAL_JBOD
#include <linux/device.h>

extern int qanp_scsi_add_sg(struct scsi_device *sdev);
...
struct dev_arr *iscsi_vjbod_arr = NULL;
#endif
...
static int is_iscsi_disk(struct scsi_device *sdp)
{
    ...
    || !strcmp(iscsi_hostt->proc_name, "iscsi_iser")
    )
        return 1;
    else
        return 0;
...
static int qnap_sd_prepare_nas_virt_info(
...
int qnap_is_iscsi_disk(
	struct scsi_device *sdev
	)
{
	struct scsi_host_template *iscsi_hostt = sdev->host->hostt;
...
static int __qnap_get_sd_dev_name(
	struct scsi_device *sdev,
	char *disk_name
	)
{
...
static int qnap_get_iscsi_vjbod_idx(
	struct scsi_device *sdev,
	int type,
	u32 *idx
	)
...
static void qnap_clear_iscsi_vjbod_idx(
	struct scsi_device *sdev,
	int type,
	u32 idx
	)
 ...
 * 1: NOT skip probe sd
 */
static int qnap_check_skip_probe_sd(
	struct scsi_device *sdev
	)
{
	int ret = 0;
...
int qnap_scsi_get_lun_capacity(
	struct scsi_device *sdev
	)
{
...
/* Patch by QNAP: Fix bug#131110: [SSD Profiling Tool] It is fail to create test for SSD in USB JBOD. */
#if defined(CONFIG_MACH_QNAPTS)
			...
			const char *buf, size_t count)
{
	...
	if (!strncmp(buf, lbp_mode[SD_LBP_UNMAP], strlen(lbp_mode[SD_LBP_UNMAP])))
		sd_config_discard(sdkp, SD_LBP_UNMAP);
	else if (!strncmp(buf, lbp_mode[SD_LBP_WS16], strlen(lbp_mode[SD_LBP_WS16])))
		sd_config_discard(sdkp, SD_LBP_WS16);
	else if (!strncmp(buf, lbp_mode[SD_LBP_WS10], strlen(lbp_mode[SD_LBP_WS10])))
		sd_config_discard(sdkp, SD_LBP_WS10);
	else if (!strncmp(buf, lbp_mode[SD_LBP_ZERO], strlen(lbp_mode[SD_LBP_ZERO])))
		sd_config_discard(sdkp, SD_LBP_ZERO);
	else if (!strncmp(buf, lbp_mode[SD_LBP_DISABLE], strlen(lbp_mode[SD_LBP_DISABLE])))
		sd_config_discard(sdkp, SD_LBP_DISABLE);
/* Patch by QNAP: Fix bug#131110: [SSD Profiling Tool] It is fail to create test for SSD in USB JBOD. */
#if defined(CONFIG_MACH_QNAPTS)
	else if (!strncmp(buf, lbp_mode[SD_LBP_SATA_TRIM], strlen(lbp_mode[SD_LBP_SATA_TRIM])))
		sd_config_discard(sdkp, SD_LBP_SATA_TRIM);
#endif
	else
		return -EINVAL;
...
#if defined(CONFIG_MACH_QNAPTS)
	int i;
	unsigned int buf_len, trans_count;
#endif
...
/* Patch by QNAP: Fix bug#131110: [SSD Profiling Tool] It is fail to create test for SSD in USB JBOD. */
	...
	case SD_LBP_SATA_TRIM:
		...
		cmd->cmd_len = 16;
		cmd->cmnd[0] = ATA_16;
		cmd->cmnd[1] = 6 << 1; 	// 6-> DMA
		cmd->cmnd[2] = 0x6; 	// bit 2-> BYT_BLOK 1:T_LENGTH is sector unit
					// bit 1,0 -->T_LENGTH 02h:transfer length is in SECTOR_COUNT
		cmd->cmnd[4] = 0x1;	// FEATURE-> bit 1: TRIM bit
		cmd->cmnd[14] = 0x6;	// COMMAND-> Data Set Management
		...
		cmd->cmnd[5] = (trans_count >> 8) & 0xff;	// COUNT-> Number of 512-byte blocks to be transferred
		cmd->cmnd[6] = trans_count & 0xff;			// COUNT-> Number of 512-byte blocks to be transferred
...
#if defined(CONFIG_MACH_QNAPTS)
				unsigned long granularity_offset =
					find_first_bit((unsigned long *)&sdkp->unmap_granularity, 32);

				printk(KERN_NOTICE "Modify BLV from SAS: sdkp->unmap_granularity=%x, OPTIMAL TRANSFER LENGTH=%x, lba_count=%x, desc_count=%x\n",
				       sdkp->unmap_granularity,
				       get_unaligned_be32(&buffer[12]),
				       lba_count ,desc_count);
...
/* Patch by QNAP: Fix bug#131110: [SSD Profiling Tool] It is fail to create test for SSD in USB JBOD. */
#if defined(CONFIG_MACH_QNAPTS)
static int sd_get_ata_identification(struct scsi_disk *sdkp, unsigned char *buffer)
{
	unsigned char cmd[16];
	struct scsi_sense_hdr sshdr;
	int the_result;
	...
	memset(cmd, 0, 16);
	cmd[0] = ATA_16;
	cmd[1] = 4 << 1;	// 3->Non-data,4->PIO Data-IN, 5->PIO Data-Out
	cmd[2] = 0x0e;		//bit 3-> T_DIR 1:transfer from ATA
	...
	if (the_result) {
		sd_print_result(sdkp, "ATA PASSTHROUGH(16) ATA Identify failed", the_result);
		return -EINVAL;
	}
...
#ifdef VIRTUAL_JBOD
	if (!qnap_is_iscsi_disk(sdp))
		__qnap_get_sd_dev_name(sdp, gd->disk_name);
#endif
#ifdef NAS_VIRTUAL_EX
	/* Jay Wei,20150122, Task #11550. */
    if (its_iscsi_device) {
