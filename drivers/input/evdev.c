			 ...
			 const struct input_value *vals, unsigned int count)
{
	...
	struct __ir_input_value *ir_input_value;
	const struct input_value *v;
#endif
...
#if defined(CONFIG_MACH_QNAPTS) && defined(QNAP_HAL)
	// Vendor ID of SuperIO IT8528  : 0x1283
	// Vendor ID of SuperIO F71869A : 0x1934
	if ((handle->dev->id.vendor == 0x1283) ||
	    (handle->dev->id.vendor == 0x1934))
	{
		hal_event.type = HAL_EVENT_ENCLOSURE;
		hal_event.arg.action = IR_INPUT;
		ir_input_value = &hal_event.arg.param.ir_input_value;
		for (v = vals; v != vals + count; v++) 
		{
			ir_input_value->type = v->type;
			ir_input_value->code = v->code;
			ir_input_value->value = v->value;
