...
int rtk_ion_sync(int fd, enum dma_data_direction dir)
{
	...
	dma_sync_sg_for_device(NULL, buffer->sg_table->sgl, buffer->sg_table->nents, dir);
	dma_buf_put(dmabuf);
	return 0;
}
...
#ifdef CONFIG_ARCH_RTD129X //20130208 charleslin: add ioctl to get physical address
	case ION_IOC_PHYS:
	{
		bool valid;
		int ret;
