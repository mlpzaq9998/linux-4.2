...
static int chv_gpio_probe(struct chv_pinctrl *pctrl, int irq)
{
	...
	*chip = chv_gpio_chip;
	...
	/*
	 * Workaround of BIOS QX47AR20 for TBS-453A
	 * Pads used for gpio-keys have no initial value in
	 * IntSel bitfield, assign here
	 */
	if(pctrl->community->uid) {
		if(!strcmp(pctrl->community->uid, "2")) {
			void __iomem *reg;
			u32 value;

			reg = chv_padreg(pctrl, 2, CHV_PADCTRL0);
			value = readl(reg);
			if(!(value & CHV_PADCTRL0_INTSEL_MASK)) {
				value |= 1 << CHV_PADCTRL0_INTSEL_SHIFT;
				writel(value, reg);
			}

			reg = chv_padreg(pctrl, 6, CHV_PADCTRL0);
			value = readl(reg);
			if(!(value & CHV_PADCTRL0_INTSEL_MASK)) {
				value |= 2 << CHV_PADCTRL0_INTSEL_SHIFT;
				writel(value, reg);
			}
