...
//Patch by QNAP: fix PCH WOL issue
#if defined(CONFIG_MACH_QNAPTS)
	ew32(WUC, E1000_WUC_PHY_WAKE | E1000_WUC_APME | E1000_WUC_APMPME);
#else
	ew32(WUC, E1000_WUC_PHY_WAKE | E1000_WUC_APME | E1000_WUC_APMPME);
#endif
////////////////////////////
...
//Patch by QNAP: fix PCH WOL issue
#if defined(CONFIG_MACH_QNAPTS)
static void wol_from_s5_init(u32 wufc)
{
	struct pci_bus *bus;
	struct pci_dev *pdev;
	u16 val=0;
	...
	if(!bus){
		...
		//
		// The power management registers are distributed within the PCI Device 31: Function 0 space.
		//
	pdev = pci_get_slot(bus,PCI_DEVFN(31, 0));
	if(!pdev){
		return;
	}
		//
		// General Power Management Configuration 3 (0xA4-0xA5)
		// Bit 13
	...
	// This bit is cleared by the RTCRST# pin.
		//
	pci_read_config_word(pdev, 0xa4, &val); //GEN_PMCON_3[13]
	if(wufc)
		val |= (1 << 13);
	else
		val &= ~(1 << 13);
	pci_write_config_word(pdev, 0xa4, val);
}
#endif
///////////////////////////////////
...
#if defined(CONFIG_MACH_QNAPTS)
	if (adapter->flags & FLAG_IS_ICH)
		if (pdev->device == E1000_DEV_ID_PCH2_LV_LM)
			wol_from_s5_init(er32(WUFC));
#endif
///////////////////////////
