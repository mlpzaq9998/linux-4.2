...
#define AQUANTIA_AQR112C_ID         0x3a1b792
...
static int al_eth_open(struct net_device *netdev)
{
	...
	if (adapter->phy_exist == false) {
		...
		if (adapter->phy_fixup_needed) {
			rc = phy_register_fixup_for_uid(AQUANTIA_AQR112C_ID, 0xffffffff,
							al_eth_aq_phy_fixup);
			if (rc)
...
static int al_eth_set_wol(struct net_device *netdev, struct ethtool_wolinfo *wol)
{
	...
	if ((adapter) && (adapter->phy_exist) && (adapter->mdio_bus)) {
		...
		if (phydev)
        {
            ...
            hal_event.arg.action = SET_PIC_WOL;
            pic_wol = &hal_event.arg.param.netlink_pic_wol;
            pic_wol->enable = wol->wolopts & WAKE_MAGIC ? 1 : 0;
            send_hal_netlink(&hal_event);
//          printk("=== QNAP Check: WOL %s\n", pic_wol->enable?"enable":"disable"); // debug
#endif
