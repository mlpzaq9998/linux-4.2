...
rtl8168_hw_init(struct net_device *dev)
{
	...
	// QNAP LED config init
	RTL_W16(CustomLED,0x0F24);
                   ...
                   void __iomem **ioaddr_out)
{
	...
	if (pdev->bus->number == 0)
		ioaddr = ioremap(0x9804f000, 1024);
	else
		ioaddr = ioremap(0x9803C000, 1024);
#else
...
rtl8168_tx_timeout(struct net_device *dev)
{
        ...
        netif_err(tp, drv, dev, "rtl8168_tx_timeout cur_tx=%d  dirty_tx=%d entry=%x status=%x!\n",tp->cur_tx,tp->dirty_tx,entry,status1);
        ...
        for (i=0;i<NUM_TX_DESC;i++)
        {
                ...
                netif_err(tp, drv, dev, "rtl8168_tx_timeout entry %x status1=%x  status2=%x addr=%lx !\n",i,status1,status2,(long unsigned int)addr);
        }
