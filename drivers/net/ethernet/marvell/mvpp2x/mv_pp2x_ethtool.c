...
static void mv_pp2x_eth_tool_get_wol(struct net_device *netdev,
		struct ethtool_wolinfo *wol)
{
	...
	if(mac->phy_dev)
		phy_ethtool_get_wol(mac->phy_dev, wol);
}

static int mv_pp2x_eth_tool_set_wol(struct net_device *netdev,
		struct ethtool_wolinfo *wol)
{
	struct mv_pp2x_port *port = netdev_priv(netdev);
	struct mv_mac_data *mac = &port->mac_data;
	...
	err = phy_ethtool_set_wol(mac->phy_dev, wol);
	if (err == -EOPNOTSUPP)
		netdev_info(netdev, "The PHY does not support set_wol, was CONFIG_MARVELL_PHY enabled?\n");
	return err;
}
...
static const struct ethtool_ops mv_pp2x_eth_tool_ops = {
	...
	.get_wol		= mv_pp2x_eth_tool_get_wol,
	.set_wol		= mv_pp2x_eth_tool_set_wol,
};
