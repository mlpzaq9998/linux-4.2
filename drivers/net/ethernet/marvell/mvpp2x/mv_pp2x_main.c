...
module_param(link_debounce, uint, S_IRUSR | S_IWUSR);
MODULE_PARM_DESC(link_debounce, "Delay link event in msecs");
...
static void _disable_active_led(unsigned long data)
{
	struct mv_pp2x_port *port = (struct mv_pp2x_port*)data;

	gpiod_set_blink(port->led_active, 0);
}

static inline void disable_active_led(struct mv_pp2x_port *port)
{
	if(port->led_active) {
		del_timer_sync(&port->active_led_timer);
		gpiod_set_blink(port->led_active, 0);
	}
}

static void trigger_active_led(struct mv_pp2x_port *port)
{
	if(port->led_active) {
		gpiod_set_blink(port->led_active, 1);
		mod_timer(&port->active_led_timer, jiffies + msecs_to_jiffies(5));
	}
}

static void link_led_change(struct mv_pp2x_port *port, int value)
{
	if(port->led_link)
		gpiod_set_value(port->led_link, value);
}
...
static irqreturn_t mv_pp2_link_change_isr(int irq, void *data)
{
	...
	if (!port->mac_data.phy_dev)
		schedule_work(&port->link_change_work);
...
static void mv_pp22_dev_link_event(struct net_device *dev)
{
	...
	if (link_is_up) {
		...
		port->mac_data.flags &= ~MV_EMAC_F_LINK_UP;
		link_led_change(port, 0);
		disable_active_led(port);
	}
}
...
void mv_pp2_link_change_work(struct work_struct *work)
{
	struct mv_pp2x_port *port = container_of(work,
			struct mv_pp2x_port, link_change_work);
	struct net_device *dev = port->dev;
	struct gop_hw *gop = &port->priv->hw.gop;
	bool link;
	int stable;
	const int stable_criteria = link_debounce;

	if (port->priv->pp2_version == PPV21)
		goto link_change_out;

	link = mv_gop110_port_is_link_up(gop, &port->mac_data);
	netdev_err(dev, "%s %d link %s stable criteria %d\n", __func__, __LINE__, link ? "up" : "down", stable_criteria);
	stable = 0;

	/* If link down triggered interrupt, then let it go
	 * immediately
	 */
	if(!link)
		goto link_down_out;

	while(stable < stable_criteria) {
		bool new_link;

		msleep(5);
		new_link = mv_gop110_port_is_link_up(gop, &port->mac_data);
	...
	*((u32 *)tx_desc + 5) = 0;
	...
	if (frags == 1) {
		...
		*/
		...
		/*
		 * Workaround from Marvell BlueHuang.
		 * Prevent 10G SFP+ to cause kernel
		 * crash.
		 */
		pool_id = -1;
		if (pool_id > -1) {
...
void mv_pp2x_start_dev(struct mv_pp2x_port *port)
{
	...
	if (port->mac_data.phy_dev) {
		...
		if (!(port->flags & MVPP2_F_LOOPBACK)) {
			...
			INIT_WORK(&port->link_change_work, mv_pp2_link_change_work);
		}
	}
...
void mv_pp2x_stop_dev(struct mv_pp2x_port *port)
{
	...
	if (port->mac_data.phy_dev)
		...
		if (!(port->flags & MVPP2_F_LOOPBACK))
			cancel_work_sync(&port->link_change_work);
}
...
static struct gpio_desc* _mv_pp2x_led_gpio_probe(struct platform_device *pdev,
		...
		const char *prop)
{
	...
	ret = of_property_read_u32_index(node, prop, 0, &gpio);
	if(!ret) {
		unsigned long flags = GPIOF_DIR_OUT;

		active_low = 0;
		of_property_read_u32_index(node, prop, 1, &active_low);
		if(active_low)
...
static void mv_pp2x_led_gpio_probe(struct platform_device *pdev,
		struct device_node *node,
		struct mv_pp2x_port *port)
{
	port->led_active = _mv_pp2x_led_gpio_probe(pdev, node, "led-active");
	port->led_link = _mv_pp2x_led_gpio_probe(pdev, node, "led-link");

	if(port->led_active)
		setup_timer(&port->active_led_timer, _disable_active_led, (unsigned long)port);
}
...
skip_tso_buffers:
	...
	mv_pp2x_led_gpio_probe(pdev, emac_node, port);
	return 0;
...
static void mv_pp2x_shutdown(struct device *dev)
{
	...
	i = 0;
	for_each_available_child_of_node(dn, port_node) {
		struct mv_pp2x_port *port = priv->port_list[i];
		struct phy_device *phydev = port->mac_data.phy_dev;
		...
		/*
		 * Marvell 1510 series phy leaks current to
		 * CPU after power down. To prevent from leakage,
		 * turn phy to isolate mode at shutdown
		 */
		phydrv = phydev->drv;
		if(phydrv->phy_id == MARVELL_PHY_ID_88E1510) {
			int old_page, val;

			old_page = phy_read(phydev, 22);
			if(old_page < 0)
				continue;
			...
			val |= (1 << 10);
			phy_write(phydev, 0, (u16)val);

			if(old_page != 0)
				phy_write(phydev, 22, (u16)old_page);
		}
