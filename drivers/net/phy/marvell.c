...
static int marvell_of_int_active_low(struct phy_device *phydev)
{
	const __be32 *paddr;
	int len, val;
	...
	paddr = of_get_property(phydev->mdio.dev.of_node,
				"marvell,int-active-low", &len);
	if (!paddr || (len < 4))
		return 1;

	val = be32_to_cpup(paddr);
...
static int m88e1318_set_wol(struct phy_device *phydev, struct ethtool_wolinfo *wol)
{
	...
	if (wol->wolopts & WAKE_MAGIC) {
		...
		temp |= marvell_of_int_active_low(phydev) ?
			MII_88E1318S_PHY_LED_TCR_INT_ACTIVE_LOW : 0;
