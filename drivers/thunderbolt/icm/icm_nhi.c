...
static void qnap_tbt_link_err_recovery(int type)
{
	NETLINK_EVT hal_event;
    
	memset(&hal_event, 0, sizeof(NETLINK_EVT));
	hal_event.type = HAL_EVENT_NET;
	hal_event.arg.action = TBT_VIRTUAL_NIC_ERR_RECOVERY;
	hal_event.arg.param.tbt_virtual_nic.err_recovery_type = type;
	//printk(KERN_ALERT "Send TBT_VIRTUAL_NIC_ERR_RECOVERY for %d\n", type);
	...
	if (nhi_ctxt && !nhi_ctxt->d0_exit) {
		...
		if (!port->net_dev) {
			...
			if (!port->net_dev) {
				...
				memset(&hal_event, 0, sizeof(NETLINK_EVT));
				hal_event.type = HAL_EVENT_NET;
				hal_event.arg.action = TBT_VIRTUAL_NIC_ADD;
