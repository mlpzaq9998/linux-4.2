...
extern int send_hal_netlink(NETLINK_EVT *evt);
			...
			enum medium_status medium_sts)
{
...
#ifdef CONFIG_MACH_QNAPTS 
		printk(KERN_ALERT "TBT negotiation_events, case MEDIUM_DISCONNECTED, send_hal_netlink to suspend. port->negotiation_status %d\n", port->negotiation_status);

		if (port->negotiation_status == MEDIUM_READY_FOR_APPROVAL)
		{
			memset(&hal_event, 0, sizeof(NETLINK_EVT));
    			hal_event.type = HAL_EVENT_NET;
	    		hal_event.arg.action = TBT_VIRTUAL_NIC_SUSPEND;
    			snprintf(hal_event.arg.param.tbt_virtual_nic.tbt_virtual_nic_name,
	    			 sizeof(hal_event.arg.param.tbt_virtual_nic.tbt_virtual_nic_name),
		    		 "%s", net_dev->name);
    			hal_event.arg.param.tbt_virtual_nic.nic_up_down_flag = 0;
	    		//printk(KERN_ALERT "Send hal_event#1 for %s %d\n", net_dev->name, hal_event.arg.param.tbt_virtual_nic.nic_up_down_flag);
	...
	case MEDIUM_CONNECTED:
		 ...
		 */
#ifdef CONFIG_MACH_QNAPTS 
		printk(KERN_ALERT "TBT negotiation_events, case MEDIUM_CONNECTED\n");
#endif
		...
		memset(&hal_event, 0, sizeof(NETLINK_EVT));
		hal_event.type = HAL_EVENT_NET;
		hal_event.arg.action = TBT_VIRTUAL_NIC_READY;
		snprintf(hal_event.arg.param.tbt_virtual_nic.tbt_virtual_nic_name,
			   sizeof(hal_event.arg.param.tbt_virtual_nic.tbt_virtual_nic_name),
		"%s", net_dev->name);
		hal_event.arg.param.tbt_virtual_nic.nic_up_down_flag = 1;
		send_hal_netlink(&hal_event);
	...
	case MEDIUM_READY_FOR_CONNECTION:
		...
		memset(&hal_event, 0, sizeof(NETLINK_EVT));
		hal_event.type = HAL_EVENT_NET;
		hal_event.arg.action = TBT_VIRTUAL_NIC_RESUME;
		snprintf(hal_event.arg.param.tbt_virtual_nic.tbt_virtual_nic_name,
				 sizeof(hal_event.arg.param.tbt_virtual_nic.tbt_virtual_nic_name),
				 "%s", net_dev->name);
		hal_event.arg.param.tbt_virtual_nic.nic_up_down_flag = 1;
		//printk(KERN_ALERT "Send hal_event#1 for %s %d\n", net_dev->name, hal_event.arg.param.tbt_virtual_nic.nic_up_down_flag);
		send_hal_netlink(&hal_event);
		printk(KERN_ALERT "TBT negotiation_events, case MEDIUM_READY_FOR_CONNECTION, send_hal_netlink to resume\n");
#endif
		break;
...
#ifdef CONFIG_MACH_QNAPTS 
    /* QNAP modified MAC address for BUG#92439, ONLY support 8 thunderbolt ports maximum!! */
	net_dev->dev_addr[0] = (0x0f << 4) | (port_num << 1);
#else
