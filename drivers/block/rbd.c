...
#define RBD_FEATURES_DATA_POOL  (1<<7)
#define RBD_FEATURES_ALL \
		(RBD_FEATURE_LAYERING | \
		 RBD_FEATURE_STRIPINGV2 | \
		 RBD_FEATURES_DATA_POOL)
				 ...
				 struct rbd_image_header_ondisk *ondisk)
{
	...
	if (first_time) {
		...
		header->data_pool_id = CEPH_NOPOOL;
...
#if defined(CONFIG_MACH_QNAPTS) && defined(CONFIG_QTS_CEPH)
EXPORT_SYMBOL(rbd_obj_notify_ack_sync);
#endif
...
EXPORT_SYMBOL(rbd_obj_notify_scsi_event_sync);
#else
/*
 * Synchronous osd object method call.  Returns the number of bytes
...
EXPORT_SYMBOL(rbd_detach_tcm_dev);
...
EXPORT_SYMBOL(rbd_attach_tcm_dev);
...
static int rbd_dev_v2_data_pool(struct rbd_device *rbd_dev)
{
	...
	ret = rbd_obj_method_sync(rbd_dev, rbd_dev->header_name,
					"rbd", "get_data_pool", NULL, 0,
					&data_pool_id, sizeof(data_pool_id));

	if (ret < 0) {
		pr_err("rbd_obj_method_sync failed\n");
		return ret;
	}
	if (ret < sizeof(data_pool_id)) {
		pr_err("invalid data_pool_id value\n");
		return -EBADMSG;
	}

	rbd_dev->header.data_pool_id = le64_to_cpu(data_pool_id);
	pr_err("retrieved data_pool_id: %lld\n", rbd_dev->header.data_pool_id);
...
static int rbd_dev_v2_snap_context(struct rbd_device *rbd_dev)
{
	...
	ret = rbd_obj_method_sync(rbd_dev, rbd_dev->header_name,
				...
				reply_buf, size);
	rbd_warn(NULL, "%s: rbd_obj_method_sync returned %d", __func__, ret);
	if (ret < 0)
		goto out;
				...
				struct rbd_spec **rbd_spec)
{
	...
	*(snap_name + len) = '\0';
	...
	copts = ceph_parse_options(options, mon_addrs,
					...
					parse_rbd_opts_token, rbd_opts, false);
	if (IS_ERR(copts)) {
		ret = PTR_ERR(copts);
		goto out_err;
	}
...
static int rbd_dev_v2_header_onetime(struct rbd_device *rbd_dev)
{
    ...
    /* support data pool */
	if (rbd_dev->header.features & RBD_FEATURES_DATA_POOL) {
			  ...
			  size_t count)
{
    ...
    /* Actually set up data pool AFTER creating the device */

    /*
     * Note: CEPH_NOPOOL != 0, so an extra check to see if
     * header.data_pool == 0 (such that it wasn't specified)
     * is required here.
     */

	if (rbd_dev->header.data_pool_id != CEPH_NOPOOL && 
        rbd_dev->header.data_pool_id != 0)
		    rbd_dev->layout.fl_pg_pool = cpu_to_le32(
			    rbd_dev->header.data_pool_id);
			     ...
			     size_t count)
{
    ...
    /*
     * Abandon data pool info from rbd_dev->layout,
     * otherwise we get warnings when tearing down requests
     */
	if (rbd_dev->header.data_pool_id != CEPH_NOPOOL)
		rbd_dev->layout.fl_pg_pool = cpu_to_le32(
			(u32) rbd_dev->spec->pool_id);
