...
static int nvme_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	/*
	 * doing mmio enabling in early nvme init could fix
	 * qm2 pcie-switch upstream port sometimes not being
	 * enabled in schedule_work, due to preemption.
	 * In other words, if qm2 upstream port mmio is not enabled,
	 * accessing nvme device registers will cause kernel exception.
	 */
	if (pci_enable_device_mem(pdev))
		return -ENOMEM;
