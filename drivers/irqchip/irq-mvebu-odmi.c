				 ...
				 unsigned int nr_irqs, void *args)
{
	struct odmi_data *odmi = NULL;
	struct irq_data *d;
	struct of_phandle_args *irq_data = args;
	struct of_phandle_args gic_data = *irq_data;
	irq_hw_number_t hwirq hwirq;
	...
	gic_data.args[1] = hwirq - 32;
	gic_data.args[2] = IRQ_TYPE_EDGE_RISING;

	ret = irq_domain_alloc_irqs_parent(domain, virq, 1, &gic_data);
	if (ret) {
