...
static int snd_card_probe(struct platform_device *devptr)
{
    ...
    int dev = devptr->id;
    int idx, err, ret;
    struct gpio audio_gpios[] = {
        { 100, GPIOF_OUT_INIT_HIGH, "MUTE" },
    };
    ...
    if ((err = snd_card_register(card)) == 0) {
        ...
        ret = gpio_request_array(audio_gpios, ARRAY_SIZE(audio_gpios));
        if(ret < 0) {
        ...
        // Off GPIO AUDIO mute
        gpio_set_value(audio_gpios[0].gpio, 1);
        printk(KERN_INFO "[AUDIO] Mute Off (gpio 100)\n");
